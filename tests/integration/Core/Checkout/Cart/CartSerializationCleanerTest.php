...
use HeyFrame\Core\Framework\Test\TestCaseBase\EventDispatcherBehaviour;
...
class CartSerializationCleanerTest extends TestCase
{
    ...
    {
        return (new Cart('test'))->add(self::payloadItem($id, $payload));
    }

    private static function coverCart(string $id, ?string $thumbnailString, bool $skipCover = false): Cart
    {
        return (new Cart('test'))->add(self::coverItem($id, $thumbnailString, $skipCover));
    }
}
