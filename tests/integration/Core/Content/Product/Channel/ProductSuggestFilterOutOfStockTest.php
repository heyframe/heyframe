...
namespace HeyFrame\Tests\Integration\Core\Content\Product\Channel;
...
use HeyFrame\Core\Content\Test\Product\Channel\Fixture\ListingTestData;
...
class ProductSuggestFilterOutOfStockTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $parent = static::getContainer()->get(Connection::class)->fetchOne(
            'SELECT LOWER(HEX(navigation_category_id)) FROM channel WHERE id = :id',
    ...
    private function createProduct(string $key, array $options, array $listingGroups): void
    {
        ...
        $data = [
            [
                ...
                'active' => true,
                'price' => [
                    ['currencyId' => Defaults::CURRENCY, 'gross' => 10],
                ],
                'categories' => [
                    ['id' => $this->categoryId],
                ],
