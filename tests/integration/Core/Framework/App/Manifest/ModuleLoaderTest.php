...
namespace HeyFrame\Tests\Integration\Core\Framework\App\Manifest;
...
class ModuleLoaderTest extends TestCase
{
    ...
    public function testLoadModulesReturnsNothingIfInstanceIdFingerprintsHaveChanged(): void
    {
        $this->registerAppsWithModules();

        $systemConfigService = static::getContainer()->get(SystemConfigService::class);
        $systemConfigService->set(InstanceIdProvider::INSTANCE_ID_SYSTEM_CONFIG_KEY_V2, (array) InstanceId::v2(Uuid::randomHex(), [
    ...
    private function validateSource(string $givenSource, string $urlPath, string $secret): void
    {
        ...
        $shopIdConfig = static::getContainer()->get(SystemConfigService::class)->get(InstanceIdProvider::INSTANCE_ID_SYSTEM_CONFIG_KEY_V2);
        static::assertIsArray($shopIdConfig);
        $shopId = InstanceId::fromSystemConfig($shopIdConfig);
        ...
        static::assertSame(static::getContainer()->getParameter('kernel.heyframe_version'), $query['sw-version']);
        ...
        static::assertArrayHasKey('heyframe-shop-signature', $query);

        $signature = $query['heyframe-shop-signature'];
        static::assertIsString($signature);
        $signedQuery = str_replace('&heyframe-shop-signature=' . $signature, '', $queryString);
