...
use HeyFrame\Core\Content\Mail\Service\AbstractMailFactory;
use HeyFrame\Core\Content\Mail\Service\AbstractMailSender;
use HeyFrame\Core\Content\Mail\Service\MailService;
...
class MailServiceTest extends TestCase
{
    ...
    /**
     * @var MockObject&EntityRepository<ChannelCollection>
     */
    ...
    protected function setUp(): void
    {
        ...
        $this->channelRepository = $this->createMock(EntityRepository::class);
    ...
    public function testSendMailSuccess(): void
    {
        ...
        $channelResult = new EntitySearchResult(
            'channel',
            1,
        ...
        $this->channelRepository->expects($this->once())->method('search')->willReturn($channelResult);

        $data = [
            'recipients' => [],
            'senderName' => 'me',
            'senderEmail' => 'me@heyframe.com',
            'subject' => 'Test email',
    ...
    public function testSendMailWithoutSenderName(): void
    {
        ...
        $email = (new Email())->subject($data['subject'])
            ->html($data['contentHtml'])
            ->text($data['contentPlain'])
            ->to('test@heyframe.com')
            ->from(new Address('test@heyframe.com'));
    ...
    public function testMailInTestModeHasNoEmptyHeaders(): void
    {
        ...
        static::assertSame(Defaults::LANGUAGE_SYSTEM, $headers->get('X-HeyFrame-Language-Id')?->getBody());
        static::assertSame($channelId, $headers->get('X-HeyFrame-Sales-Channel-Id')?->getBody());
