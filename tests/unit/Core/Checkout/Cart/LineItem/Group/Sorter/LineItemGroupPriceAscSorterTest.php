...
namespace HeyFrame\Tests\Unit\Core\Checkout\Cart\LineItem\Group\Sorter;
...
class LineItemGroupPriceAscSorterTest extends TestCase
{
    ...
    {
        static::assertSame('PRICE_ASC', $this->sorter->getKey());
    }
    ...
    public function testSortPriceASC(): void
    {
        ...
        static::assertSame($p2->getId(), $sortedItems->getElements()[0]->getId());
        static::assertSame($p1->getId(), $sortedItems->getElements()[1]->getId());
        static::assertSame($p3->getId(), $sortedItems->getElements()[2]->getId());
    }
