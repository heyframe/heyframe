 ...
 * It's only used for HeyFrame plugins. This solves the problem of having multiple Vue instances in the same project.
 *
 * Inspired by: https://github.com/crcong/vite-plugin-externals/
 */
export default function viteExternalsPlugin(): Plugin {
    return {
        name: 'heyframe-vite-plugin-vue-globals',
        ...
        async config(config) {
            ...
            const cachePath = path.join(process.cwd(), 'node_modules', '.heyframe-vite-plugin-vue-globals');
            ...
            await writeFile(vueJsCachePath, `module.exports = window['HeyFrame']?.['Vue']`);
