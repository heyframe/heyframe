...
import {
    ...
    MtToast,
    MtTextEditor,
} from '@heyframe-ag/meteor-component-library';
...
import '../../src/app/store/heyframe-apps.store';
...
// Add services
HeyFrame.Service().register('acl', () => aclService);
HeyFrame.Service().register('feature', () => feature);
HeyFrame.Feature = HeyFrame.Service('feature');
HeyFrame.Service().register('repositoryFactory', () => repositoryFactory);

// Provide all services
HeyFrame.Service().list().forEach(serviceKey => {
    config.global.provide[serviceKey] = HeyFrame.Service(serviceKey);
});

// Set important functions for HeyFrame Core
...
// Prepare Context
HeyFrame.Store.get('context').api.installationPath = 'installationPath';
HeyFrame.Store.get('context').api.apiPath = '/api';
HeyFrame.Store.get('context').api.apiResourcePath = '/api/v3';
HeyFrame.Store.get('context').api.assetsPath = '';
HeyFrame.Store.get('context').api.languageId = '2fbb5fe2e29a4d70aa5854ce7ce3e20b';
HeyFrame.Store.get('context').api.inheritance = false;
HeyFrame.Store.get('context').api.systemLanguageId = '2fbb5fe2e29a4d70aa5854ce7ce3e20b';
HeyFrame.Store.get('context').api.liveVersionId = '0fa91ce3e96a4bc2be4bd9ce752c3425';
HeyFrame.Context.api.authToken = {
...
const directiveRegistry = HeyFrame.Directive.getDirectiveRegistry();
