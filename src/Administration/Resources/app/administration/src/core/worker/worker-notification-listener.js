...
const { Application, WorkerNotification } = HeyFrame;
...
class WorkerNotificationListener {
    ...
    start() {
        if (!HeyFrame.Context.app.config.adminWorker.enableQueueStatsWorker) {
            return;
        }
    ...
    setupIntervalWatcher() {
        ...
        HeyFrame.Vue.watch(
            HeyFrame.Store.get('notification').workerProcessPollInterval,
    ...
    _checkQueue() {
        ...
        const client = HeyFrame.Application.getContainer('init').httpClient;
        const headers = {
            headers: {
                Authorization: `Bearer ${HeyFrame.Service('loginService').getToken()}`,
            },
        };
        ...
        client.get('/_info/queue.json', headers).then((res) => {
            ...
            if (this._isRunning) {
                this._interval = HeyFrame.Store.get('notification').workerProcessPollInterval;
                this._timeoutId = setTimeout(this._checkQueue.bind(this), this._interval);
            }
        });
    }
    ...
    runNotificationMiddleware(response) {
        ...
        const middlewareParams = {
            ...
            notification: {
                create: (notification) => {
                    return HeyFrame.Store.get('notification').createNotification(notification);
                },
                update: (notification) => {
                    return HeyFrame.Store.get('notification').updateNotification(notification);
                },
            },
            queue,
        };
