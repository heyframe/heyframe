...
describe('core/application.js', () => {
    const originalInjectJs = HeyFrame.Application.injectJs;
    const originalInjectIframe = HeyFrame.Application.injectIframe;
    const originalNodeEnv = process.env.NODE_ENV;

    beforeEach(() => {
        jest.clearAllMocks();
        HeyFrame.Application.injectJs = originalInjectJs;
        HeyFrame.Application.injectIframe = originalInjectIframe;
        process.env.NODE_ENV = originalNodeEnv;
        HeyFrame.Context.app.config.bundles = {};
    ...
    it("should be error tolerant if loading a plugin's files fails", async () => {
        ...
        HeyFrame.Application.injectJs = async () => {
            throw new Error('Inject js fails');
        };

        const result = await HeyFrame.Application.injectPlugin({
            js: ['some.js'],
        });
    ...
    it('should call swagCommercial before any other plugins', async () => {
        ...
        HeyFrame.Application.injectPlugin = async (plugin) => {
            callOrder.js.push(plugin.js);
            callOrder.css.push(plugin.css);
        };

        // load all plugins
        await HeyFrame.Application.loadPlugins();
    ...
    it('should load plugins correctly in prod', async () => {
        ...
        HeyFrame.Application.injectIframe = jest.fn();
        ...
        expect(HeyFrame.Application.injectIframe).toHaveBeenCalledWith({
    ...
    it('should load plugins correctly in watch with all permissions', async () => {
        ...
        expect(HeyFrame.Store.get('extensions').extensionsState['test-plugin']).toEqual({
            name: 'test-plugin',
            baseUrl: 'http://localhost:8000/bundles/testplugin/administration/',
            permissions: {
                additional: ['*'],
