...
export default function createLoginService(
    ...
    function verifyUserToken(password: string): Promise<string> {
        ...
        return verifyUserByUsername(HeyFrame.Store.get('session').currentUser?.username ?? '', password)
            .then(({ access }) => {
                if (HeyFrame.Utils.types.isString(access)) {
                    return access;
                }
    ...
    function loginByUsername(user: string, pass: string): Promise<AuthObject> {
        return httpClient
            ...
            .then((response) => {
                HeyFrame.Service('userActivityService').updateLastUserActivity();
    ...
    function lastActivityOverThreshold(): boolean {
        const lastActivity = HeyFrame.Service('userActivityService').getLastUserActivity().getTime();
    ...
    function shouldConsiderUserActivity(): boolean {
        ...
        const devEnv = HeyFrame.Context.app.environment === 'development';
    ...
    function forwardLogout(isInactivityLogout: boolean, shouldRedirect: boolean): void {
        ...
        const router = HeyFrame.Application.view.router as null | Router;
        if (router) {
            const id = HeyFrame.Utils.createId();
            ...
            if (isInactivityLogout && shouldRedirect) {
                ...
                void html2canvas(document.querySelector('#app'), {
                    ...
                    sessionStorage.setItem('lastKnownUser', HeyFrame.Store.get('session').currentUser?.username ?? '');
