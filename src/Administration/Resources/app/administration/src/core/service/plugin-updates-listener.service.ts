...
export default function addPluginUpdatesListener(loginService: LoginService, serviceContainer: ServiceContainer): void {
    ...
    function createUpdatesAvailableNotification(): void {
        const root = HeyFrame.Application.getApplicationRoot();
        ...
        void HeyFrame.Store.get('notification').createNotification(notification);
    }
    ...
    HeyFrame.Vue.watch(useSession().currentUser, (newValue) => {
        if (!newValue) {
            return;
        }
