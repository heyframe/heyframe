...
import { updateSubscriber, register, handleGet } from '@heyframe-ag/meteor-admin-sdk/es/data';
import { get, debounce, cloneDeepWith } from 'lodash';
import type { App } from 'vue';
import { selectData } from '@heyframe-ag/meteor-admin-sdk/es/_internals/data/selectData';
import MissingPrivilegesError from '@heyframe-ag/meteor-admin-sdk/es/_internals/privileges/missing-privileges-error';
...
handleGet((data, additionalOptions) => {
    ...
    if (registeredDataSet.deprecated) {
        ...
        if (process.env.NODE_ENV !== 'production') {
            HeyFrame.Utils.debug.error(...debugArgs);
        } else {
            HeyFrame.Utils.debug.warn(...debugArgs);
        }
    }
    ...
    updateSubscriber(id, (value) => {
        ...
        function setObject(transferObject: transferObject, prePath: string | null = null): void {
            ...
            Object.keys(transferObject).forEach((property) => {
                ...
                if (
                    // @ts-expect-error
                    HeyFrame.Utils.hasOwnProperty(transferObject[property], 'getDraft', this) &&
                    // @ts-expect-error
                    typeof transferObject[property].getDraft === 'function'
                ) {
                    setObject(
                        {
                            [property]: HeyFrame.Utils.object.cloneDeep(transferObject[property]),
                        },
                        realPath,
                    );
                ...
                HeyFrame.Utils.object.get(scope, parsedPath.pathToLastSegment)[parsedPath.lastSegment] =
        ...
        if (path.includes('.')) {
            ...
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
            HeyFrame.Utils.object.get(scope, newPath)[lastPath] = value.data;
