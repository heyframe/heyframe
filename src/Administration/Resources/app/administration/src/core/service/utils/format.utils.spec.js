...
describe('src/core/service/utils/format.utils.js', () => {
    ...
    describe('date', () => {
        const setLocale = (locale) => {
            jest.spyOn(HeyFrame.Application.getContainer('factory').locale, 'getLastKnownLocale').mockImplementation(
                () => locale,
            );
        };
        const setTimeZone = (timeZone) => HeyFrame.Store.get('session').setCurrentUser({ timeZone });
    ...
    describe('currency', () => {
        const currencyFilter = HeyFrame.Utils.format.currency;
        ...
        it('should fallback to decimal when the currency ISO code is invalid', async () => {
            HeyFrame.Context.app.systemCurrencyISOCode = 'INVALID_EXAMPLE_CURRENCY_CODE';
