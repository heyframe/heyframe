...
export function currency(val: number, sign: string, decimalPlaces: number, additionalOptions: CurrencyOptions = {}): string {
    ...
    const opts = {
        style: 'currency',
        currency: sign || (HeyFrame.Context.app.systemCurrencyISOCode as string),
        ...decimalOpts,
        ...additionalOptions,
    };
    ...
    try {
        result = val.toLocaleString(
            additionalOptions.language ?? HeyFrame.Store.get('session').currentLocale ?? 'en-US',
...
export function date(val: string, options: DateFilterOptions = {}): string {
    ...
    const userTimeZone = HeyFrame?.Store?.get('session')?.currentUser?.timeZone ?? 'UTC';
