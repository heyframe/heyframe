...
import type { MessageDataType, HandleMethod, BaseMessageOptions } from '@heyframe-ag/meteor-admin-sdk/es/channel';
import { handle as sdkHandle } from '@heyframe-ag/meteor-admin-sdk/es/channel';
import type { HeyFrameMessageTypes } from '@heyframe-ag/meteor-admin-sdk/es/message-types';
...
export default {
    ...
    handle: <MESSAGE_TYPE extends keyof HeyFrameMessageTypes>(
        type: MESSAGE_TYPE,
        ...
        const aclHook = (
            ...
            return new Promise((resolve, reject) => {
                ...
                const missingPrivileges = data.privileges!.filter((p) => !HeyFrame.Service('acl').can(p));
                if (missingPrivileges.length > 0) {
                    reject(new MissingPrivilegesError(type, missingPrivileges));
                } else {
                    const result = method(data, additionalInformation);

                    if (isPromise<HeyFrameMessageTypes[MESSAGE_TYPE]['responseType']>(result)) {
