...
class ApplicationBootstrapper {
     ...
     *
     * @example
     * HeyFrame.Application.addFactory('module', (container) => {
     ...
     *
     * @example
     * HeyFrame.Application.addFactoryMiddleware((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * HeyFrame.Application.addFactoryMiddleware('module', (service, next) => {
     ...
     *
     * @example
     * HeyFrame.Application.addFactoryDecorator((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * HeyFrame.Application.addFactoryDecorator('module', (service, next) => {
     *    // Do something with the service
     *    next();
     * });
     */
     ...
     *
     * @example
     * HeyFrame.Application.addInitializer('httpClient', (container) => {
     ...
     *
     * @example
     * HeyFrame.Application.addServiceProvider('productService', (container) => {
    ...
    registerApiContext(context: ContextStore['api']): ApplicationBootstrapper {
        HeyFrame.Context.api = HeyFrame.Classes._private.ApiContextFactory(context);
    ...
    registerAppContext(context: ContextStore['app']): ApplicationBootstrapper {
        HeyFrame.Context.app = HeyFrame.Classes._private.AppContextFactory(context);
     ...
     *
     * @example
     * HeyFrame.Application.addServiceProviderMiddleware((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * HeyFrame.Application.addServiceProviderMiddleware('productService', (service, next) => {
     *    // Do something with the service
     *    next();
     * });
     */
    ...
    initializeFeatureFlags(): ApplicationBootstrapper {
        const features = HeyFrame.Context.app.features;
        ...
        HeyFrame.Feature.init(features);
     ...
     *
     * @example
     * HeyFrame.Application.addServiceProviderDecorator((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * HeyFrame.Application.addServiceProviderDecorator('productService', (service, next) => {
     *    // Do something with the service
     *    next();
     * });
     */
    ...
    bootFullApplication(): Promise<void | ApplicationBootstrapper> {
        ...
        return this.initializeInitializers(initPreContainer, '-pre')
            ...
            .then(() => Promise.all(HeyFrame.Plugin.getBootPromises()))
            .then(() => {
                if (!this.view) {
                    return Promise.reject();
                }
    ...
    createApplicationRoot(): Promise<ApplicationBootstrapper> {
        ...
        if (HeyFrame.Context.app.environment === 'testing') {
    ...
    private async loadPlugins(): Promise<(unknown[] | null)[]> {
        ...
        if (isDevelopmentMode) {
            ...
            if (HeyFrame.Utils.object.hasOwnProperty(plugins, 'metadata')) {
                delete plugins.metadata;
            }
        } else {
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
            plugins = HeyFrame.Context.app.config.bundles as bundlesPluginResponse;
        }
    ...
    private injectIframe({
        ...
        const bundles = HeyFrame.Context.app.config.bundles;
        ...
        HeyFrame.Store.get('extensions').addExtension(extension);
    }
}
