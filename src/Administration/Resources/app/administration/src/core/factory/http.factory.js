...
/**
 * Determines the different status codes and creates a matching error via HeyFrame.State
 * @param {Number} status
 * @param {Array} errors
 * @param {Object} error
 * @param {Object} data
...
function handleErrorStates({status, errors, error = null, data}) {
    ...
    if (status === 403) {
        ...
        missingPrivilegeErrors.forEach((missingPrivilegeError) => {
            ...
            HeyFrame.Store.get('notification').createNotification({
                ...
                autoClose: false,
                growl: true,
                title: HeyFrame.Snippet.tc('global.error-codes.FRAMEWORK__MISSING_PRIVILEGE_ERROR'),
                message: `${HeyFrame.Snippet.tc('sw-privileges.error.description')} <br> ${missingPrivilegesMessage}`,
            });
        });
    }
    ...
    if (
        ...
        HeyFrame.Store.get('notification').createNotification({
            ...
            autoClose: false,
            growl: true,
            title: HeyFrame.Snippet.tc('sw-extension.errors.storeSessionExpired.title'),
            message: HeyFrame.Snippet.tc('sw-extension.errors.storeSessionExpired.message'),
            actions: [
                {
                    label: HeyFrame.Snippet.tc('sw-extension.errors.storeSessionExpired.actionLabel'),
                    method: () => {
    ...
    if (status === 409) {
        if (errors[0].code === 'FRAMEWORK__DELETE_RESTRICTED') {
            ...
            blockingEntities = parameters.usages.reduce((message, usageObject) => {
                ...
                const timesSnippet = HeyFrame.Snippet.tc('global.default.xTimesIn', times);
                const blockingEntitiesSnippet = HeyFrame.Snippet.tc(`global.entities.${usageObject.entityName}`, times[1]);
                return `${message}<br>${timesSnippet} <b>${blockingEntitiesSnippet}</b>`;
            }, '');

            HeyFrame.Store.get('notification').createNotification({
                variant: 'error',
                title: HeyFrame.Snippet.tc('global.default.error'),
                message: `${HeyFrame.Snippet.tc('global.notification.messageDeleteFailed', 3, {
                    entityName: HeyFrame.Snippet.tc(`global.entities.${entityName}`),
                })}${blockingEntities}`,
            });
        }
    }
            ...
            if (status === 401) {
                ...
                return new Promise((resolve, reject) => {
                    tokenHandler.subscribe(
                        ...
                        (err) => {
                            if (!HeyFrame.Application.getApplicationRoot()) {
                                reject(err);
...
function tracingInterceptor(client) {
    ...
    if (process.env.NODE_ENV !== 'test') {
        client.interceptors.request.use((config) => {
            const currentRoute = HeyFrame?.Application?.view?.router?.currentRoute?.value?.name;

            if (currentRoute) {
                config.headers['heyframe-admin-active-route'] = currentRoute;
            }
