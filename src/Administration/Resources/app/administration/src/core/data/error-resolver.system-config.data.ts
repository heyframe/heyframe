...
export default class ErrorResolverSystemConfig {
    ...
    public cleanWriteErrors() {
        void HeyFrame.Store.get('error').resetApiErrors();
    }

    private reduceErrorsByWriteIndex(errors: ApiError[]) {
        const writeErrors: {
            systemError: HeyFrameError[];
            apiError: {
                [key: string]: HeyFrameError;
            };
        } = {
            systemError: [],
            apiError: {},
        ...
        errors.forEach((current) => {
            if (!current.source || !current.source.pointer) {
                const systemError = new HeyFrameError({
                    code: current.code,
                    meta: current.meta,
            ...
            segments.reduce((pointer: { [key: string]: Partial<HeyFrameError> }, segment, index) => {
                // skip translations
                ...
                if (index === lastIndex) {
                    pointer[segment] = new HeyFrameError(current);
                } else {
                    pointer[segment] = {};
                }
    ...
    private addSystemErrors(errors: HeyFrameError[]) {
        errors.forEach((error) => {
            void HeyFrame.Store.get('error').addSystemError({ error });
        });
    }

    private handleErrors(errors: { [key: string]: HeyFrameError }) {
        Object.keys(errors).forEach((key: string) => {
            void HeyFrame.Store.get('error').addApiError({
