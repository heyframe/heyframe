...
export default class ErrorResolver {
    constructor() {
        this.EntityDefinition = HeyFrame.EntityDefinition;
        this.HeyFrameError = HeyFrame.Classes.HeyFrameError;
        this.merge = HeyFrame.Utils.object.merge;
    }

    resetApiErrors() {
        return HeyFrame.Store.get('error').resetApiErrors();
    }
    ...
    handleDeleteError(errors) {
        errors.forEach(({ error, entityName, id }) => {
            const heyframeError = new this.HeyFrameError(error);
            HeyFrame.Store.get('error').addSystemError({
                error: heyframeError,
            });
    ...
    reduceErrorsByWriteIndex(errors) {
        ...
        errors.forEach((current) => {
            if (!current.source || !current.source.pointer) {
                writeErrors.system.push(new this.HeyFrameError(current));
                return;
            }
            ...
            segments.reduce((pointer, segment, index) => {
                ...
                if (index === lastIndex) {
                    pointer[segment] = new this.HeyFrameError(current);
                } else {
                    pointer[segment] = {};
                }
    ...
    addSystemErrors(systemErrors) {
        systemErrors.forEach((error) => {
            HeyFrame.Store.get('error').addSystemError(error);
        });
    }
    ...
    resolveError(fieldName, error, definition, entity, changeset) {
        ...
        if (!(error instanceof this.HeyFrameError)) {
            error = new this.HeyFrameError(error);
        }
    ...
    buildAssociationChangeset(entity, changeset, error, associationName) {
        if (!changeset || !HeyFrame.Utils.object.hasOwnProperty(changeset, associationName)) {
            HeyFrame.Store.get('error').addApiError({
    ...
    resolveJsonFieldError(basePath, error) {
        Object.keys(error).forEach((fieldName) => {
            ...
            if (error[fieldName] instanceof this.HeyFrameError) {
                HeyFrame.Store.get('error').addApiError({
                    expression: path,
