...
describe('src/core/data/error-resolver.data', () => {
    ...
    beforeEach(() => {
        ...
        jest.spyOn(HeyFrame.Store.get('error'), 'resetApiErrors');
        jest.spyOn(HeyFrame.Store.get('error'), 'addApiError');
        jest.spyOn(HeyFrame.Store.get('error'), 'addSystemError');
    ...
    describe('resetApiErrors', () => {
        it('should dispatches "error/resetApiErrors" action', () => {
            ...
            expect(HeyFrame.Store.get('error').resetApiErrors).toHaveBeenCalledTimes(1);
        });
    });
    ...
    describe('handleWriteErrors', () => {
        ...
        it('should handles write errors and adds system errors', () => {
            ...
            expect(HeyFrame.Store.get('error').addApiError).toHaveBeenCalledTimes(2);
            expect(HeyFrame.Store.get('error').addSystemError).toHaveBeenCalledTimes(1);
            expect(HeyFrame.Store.get('error').addApiError).toHaveBeenNthCalledWith(1, {
                expression: expect.anything(),
                error: expect.any(HeyFrame.Classes.HeyFrameError),
            });
            expect(HeyFrame.Store.get('error').addApiError).toHaveBeenNthCalledWith(2, {
                expression: expect.anything(),
                error: expect.any(HeyFrame.Classes.HeyFrameError),
            });
            expect(HeyFrame.Store.get('error').addSystemError).toHaveBeenCalledWith(
                expect.any(HeyFrame.Classes.HeyFrameError),
            );
        });

        it('should convert to HeyFrameError', () => {
            const errors = [
                { source: { pointer: '/0/firstName' }, code: 'CODE1' },
            ];
            ...
            expect(HeyFrame.Store.get('error').addApiError).toHaveBeenCalledWith({
                expression: expect.anything(),
                error: expect.any(HeyFrame.Classes.HeyFrameError),
            });
        });
    ...
    describe('handleDeleteError', () => {
        it('should handle delete errors and add system errors and api errors', () => {
            ...
            expect(HeyFrame.Store.get('error').addSystemError).toHaveBeenCalledTimes(2);
            expect(HeyFrame.Store.get('error').addApiError).toHaveBeenCalledTimes(2);
            expect(HeyFrame.Store.get('error').addSystemError).toHaveBeenCalledWith({
                error: expect.any(HeyFrame.Classes.HeyFrameError),
            });
