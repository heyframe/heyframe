    ...
    defineComponent({
        methods: {
            placeholder<EntityName extends keyof EntitySchema.Entities>(
                ...
                if (HeyFrame.Utils.types.isString(entity[field]) && entity[field].length > 0) {
                    return entity[field];
                }

                // Return the field from parent translation if set
                const parentLanguageId = HeyFrame.Context.api.language ? HeyFrame.Context.api.language.parentId : null;
