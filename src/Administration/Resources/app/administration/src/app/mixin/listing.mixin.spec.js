...
async function createWrapper({
    ...
    return mount(
        ...
        `,
            mixins: [
                HeyFrame.Mixin.getByName('listing'),
            ],
            data() {
                return {
                    ...defaultData,
...
describe('src/app/mixin/listing.mixin.ts', () => {
    ...
    beforeEach(async () => {
        ...
        if (originalWarn) {
            HeyFrame.Utils.debug.warn = originalWarn;
        } else {
            originalWarn = HeyFrame.Utils.debug.warn;
        }
    });
    ...
    it('should throw a console warning when no getList method ist defined', async () => {
        ...
        expect(HeyFrame.Utils.debug.warn).toHaveBeenCalledWith(
            'Listing Mixin',
