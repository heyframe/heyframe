...
async function createWrapper(attrs = {}) {
    return mount(
        ...
        `,
            mixins: [
                HeyFrame.Mixin.getByName('remove-api-error'),
            ],
            data() {
                return {
                    name: 'sw-mock-field',
...
describe('src/app/mixin/remove-api-error.mixin.ts', () => {
    ...
    beforeEach(async () => {
        if (originalDispatch) {
            Object.defineProperty(HeyFrame.State, 'dispatch', {
                value: originalDispatch,
            });
        } else {
            originalDispatch = HeyFrame.State.dispatch;
        }
    ...
    it('should call  removeApiError on value change', async () => {
        ...
        const errorStore = HeyFrame.Store.get('error');
        ...
        expect(HeyFrame.Store.get('error').removeApiError).toHaveBeenCalledWith('self.link');
    });
});
