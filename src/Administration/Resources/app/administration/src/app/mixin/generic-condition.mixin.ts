    ...
    defineComponent({
        ...
        computed: {
            config(): Config {
                ...
                const config = HeyFrame.Store.get('ruleConditionsConfig').getConfigForType(this.condition.type as string) as
                    | Config
                    | undefined;
            ...
            currentError() {
                ...
                Object.values(this.config.fields).forEach((config) => {
                    ...
                    const errorProperty = HeyFrame.Store.get('error').getApiError(
                        // @ts-expect-error
        ...
        methods: {
            getBind(field: Field) {
                const fieldClone = HeyFrame.Utils.object.cloneDeep(field);
