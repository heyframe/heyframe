...
import type { ApiContext } from '@heyframe-ag/meteor-admin-sdk/es/_internals/data/EntityCollection';
...
export default function initializeWorker() {
    ...
    const context = HeyFrame.Context.app;
    const workerNotificationFactory = HeyFrame.Application.getContainer('factory').workerNotification;
    const configService = HeyFrame.Service('configService');
        ...
        return configService.getConfig().then((response) => {
            Object.entries(response as ContextAppConfig).forEach(
                ([
                    ...
                    HeyFrame.Store.get('context').addAppConfigValue({
            ...
            enableWorkerNotificationListener(loginService, HeyFrame.Context.api);
            ...
            if (context.config.adminWorker?.enableAdminWorker && !enabled) {
                enableAdminWorker(loginService, HeyFrame.Context.api, context.config.adminWorker);
            }
        });
    }
...
function enableAdminWorker(
    ...
    const importExportService = HeyFrame.Service('importExport');
...
function registerThumbnailMiddleware(factory: typeof WorkerNotificationFactory) {
    ...
    factory.register('EsIndexingMessage', {
        name: 'HeyFrame\\Elasticsearch\\Framework\\Indexing\\IndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('generateThumbnailsMessage', {
        name: 'HeyFrame\\Core\\Content\\Media\\Message\\GenerateThumbnailsMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('PromotionIndexingMessage', {
        name: 'HeyFrame\\Core\\Checkout\\Promotion\\DataAbstractionLayer\\PromotionIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('ProductStreamIndexingMessage', {
        name: 'HeyFrame\\Core\\Content\\ProductStream\\DataAbstractionLayer\\ProductStreamIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification(
                'productStream',
                ids,
    ...
    factory.register('CategoryIndexingMessage', {
        name: 'HeyFrame\\Core\\Content\\Category\\DataAbstractionLayer\\CategoryIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification(
                'category',
                ids,
    ...
    factory.register('MediaIndexingMessage', {
        name: 'HeyFrame\\Core\\Content\\Media\\DataAbstractionLayer\\MediaIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification(
                'media',
                ids,
    ...
    factory.register('RuleIndexingMessage', {
        name: 'HeyFrame\\Core\\Content\\Rule\\DataAbstractionLayer\\RuleIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification(
                'rule',
                ids,
    ...
    factory.register('ProductIndexingMessage', {
        name: 'HeyFrame\\Core\\Content\\Product\\DataAbstractionLayer\\ProductIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification(
                'product',
                ids,
    ...
    factory.register('ElasticSearchIndexingMessage', {
        name: 'HeyFrame\\Elasticsearch\\Framework\\Indexing\\ElasticsearchIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification(
                'esIndexing',
                ids,
    ...
    factory.register('ImportExportMessage', {
        name: 'HeyFrame\\Core\\Content\\ImportExport\\Message\\ImportExportMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('FlowIndexingMessage', {
        name: 'HeyFrame\\Core\\Content\\Flow\\Indexing\\FlowIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('NewsletterRecipientIndexingMessage', {
        name: 'HeyFrame\\Core\\Content\\Newsletter\\DataAbstractionLayer\\NewsletterRecipientIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
