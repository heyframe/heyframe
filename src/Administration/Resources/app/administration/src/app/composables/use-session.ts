...
async function setAdminLocale(locale: string): Promise<void> {
    ...
    const localeToLanguageService = HeyFrame.Service('localeToLanguageService') as LocaleToLanguageService;
    return localeToLanguageService.localeToLanguage(locale).then((languageIdFromService) => {
        ...
        HeyFrame.Application.getContainer('factory').locale.storeCurrentLocale(locale);
    });
}
...
function setAdminLocaleState({
    ...
    if (!locales.find((l) => l === locale)) {
        HeyFrame.Utils.debug.warn('SessionStore', `Locale ${locale} not registered at store`);
        return;
    }
