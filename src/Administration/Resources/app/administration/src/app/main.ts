...
import addHeyFrameUpdatesListener from 'src/core/service/heyframe-updates-listener.service';
...
import HeyFrameDiscountCampaignService from 'src/app/service/discount-campaign.service';
...
import '@heyframe-ag/meteor-component-library/styles.css';
// eslint-disable-next-line import/no-unresolved
...
Application.addServiceProvider('feature', () => {
    ...
    .addServiceProvider('loginService', () => {
        ...
        const loginService = LoginService(initContainer.httpClient, HeyFrame.Context.api);

        addPluginUpdatesListener(loginService, serviceContainer);
        addHeyFrameUpdatesListener(loginService, serviceContainer);
        return loginService;
    })
    ...
    .addServiceProvider('extensionHelperService', () => {
        return new ExtensionHelperService({
            // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
            extensionStoreActionService: HeyFrame.Service('extensionStoreActionService'),
        });
    })
    ...
    .addServiceProvider('localeHelper', () => {
        return new LocaleHelperService({
            HeyFrame: HeyFrame,
            localeRepository: HeyFrame.Service('repositoryFactory').create('locale'),
            // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
            snippetService: HeyFrame.Service('snippetService'),
            // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
    ...
    .addServiceProvider('filterService', () => {
        return new FilterService({
            userConfigRepository: HeyFrame.Service('repositoryFactory').create('user_config'),
        });
    })
    ...
    .addServiceProvider('heyframeDiscountCampaignService', () => {
        return new HeyFrameDiscountCampaignService();
    })
