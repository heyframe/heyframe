...
describe('src/app/component/form/sw-datepicker', () => {
    ...
    beforeEach(async () => {
        HeyFrame.Store.get('session').setCurrentUser({ timeZone: 'UTC' });
    });

    afterAll(() => {
        HeyFrame.Store.get('session').setCurrentUser(currentUser);
    });
    ...
    it('should use the admin locale', async () => {
        HeyFrame.Store.get('session').currentLocale = 'de-DE';
        wrapper = await createWrapper();
    ...
    it.each([
        ...
        async ({ dateType, timeZone, expectedTimeZone }) => {
            HeyFrame.Store.get('session').setCurrentUser({ timeZone: timeZone });
    ...
    it('should not convert the date when a timezone is set and dateType is date', async () => {
        HeyFrame.Store.get('session').setCurrentUser({ timeZone: 'Europe/Berlin' });
    ...
    it('should support other locales formats', async () => {
        HeyFrame.Store.get('session').currentLocale = 'en-US';
        wrapper = await createWrapper({});
        ...
        HeyFrame.Store.get('session').currentLocale = 'en-UK';
