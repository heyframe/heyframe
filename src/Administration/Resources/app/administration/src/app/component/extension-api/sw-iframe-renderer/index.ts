...
export default HeyFrame.Component.wrapComponentConfig({
    ...
    created() {
        this.heightHandler = HeyFrame.ExtensionAPI.handle('locationUpdateHeight', ({ height, locationId }) => {
            if (locationId === this.locationId) {
                this.locationHeight = Number(height) ?? null;
            }
        });

        this.urlHandler = HeyFrame.ExtensionAPI.handle(
            'locationUpdateUrl',
    ...
    computed: {
        ...
        componentName(): string | undefined {
            return HeyFrame.Store.get('sdkLocation').locations[this.locationId];
        },

        extension(): Extension | undefined {
            const extensions = HeyFrame.Store.get('extensions').extensionsState;
