...
describe('components/form/sw-url-field', () => {
    ...
    it('properly detects SSL', async () => {
        const SSL_URL = 'https://heyframe.com';
        const NON_SSL_URL = 'http://heyframe.com';
        const URL_WITHOUT_PROTOCOL = 'heyframe.com';
    ...
    it('removes any protocol', async () => {
        const HTTP_URL = 'http://heyframe.com';
        const HTTPS_URL = 'https://heyframe.com';
        const FILE_URL = 'file://heyframe.com';
        const EXPECTED_URL = 'heyframe.com';
    ...
    it('allows empty values', async () => {
        const INITIAL_URL = 'https://heyframe.com';
        const URL_WITHOUT_PROTOCOL = 'heyframe.com';
        const EXPECTED_URL = '';
    ...
    it('should update empty values', async () => {
        const wrapper = await createWrapper({
            props: {
                value: 'https://heyframe.com',
            },
        });
        await flushPromises();

        const input = wrapper.find('input');
        expect(input.element.value).toBe('heyframe.com');
    ...
    it('adds the trailing slash if the prop is set', async () => {
        ...
        await input.setValue('heyframe.com');
        await input.trigger('blur');
        await nextTick();
        expect(wrapper.vm.currentUrlValue).toBe('heyframe.com/');
        expect(wrapper.emitted('update:value')).toStrictEqual([
            ['https://heyframe.com/'],
        ]);
    });
