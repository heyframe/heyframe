...
describe('sw-app-wrong-app-url-modal', () => {
    ...
    beforeAll(() => {
        ...
        removeNotificationSpy = jest.spyOn(HeyFrame.Store.get('notification'), 'removeNotification');
    });

    it('should show modal', async () => {
        HeyFrame.Store.get('context').app.config.settings.appUrlReachable = false;
        HeyFrame.Store.get('context').app.config.settings.appsRequireAppUrl = true;
    ...
    it('should not show modal if APP_URL is reachable', async () => {
        HeyFrame.Store.get('context').app.config.settings.appUrlReachable = true;
        HeyFrame.Store.get('context').app.config.settings.appsRequireAppUrl = true;
        localStorage.removeItem(STORAGE_KEY_WAS_WRONG_APP_MODAL_SHOWN);
    ...
    it('should not show modal if no apps are require app url, but it should show notification', async () => {
        HeyFrame.Store.get('context').app.config.settings.appUrlReachable = false;
        HeyFrame.Store.get('context').app.config.settings.appsRequireAppUrl = false;
        localStorage.removeItem(STORAGE_KEY_WAS_WRONG_APP_MODAL_SHOWN);
