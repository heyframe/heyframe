...
export default {
    ...
    computed: {
        notifications() {
            return Object.values(HeyFrame.Store.get('notification').growlNotifications);
        },
    ...
    methods: {
        onClose(notification) {
            HeyFrame.Store.get('notification').removeGrowlNotification(notification);
        },

        handleAction(action, notification) {
            // Allow external links for example to the heyframe account or store
            if (HeyFrame.Utils.string.isUrl(action.route)) {
