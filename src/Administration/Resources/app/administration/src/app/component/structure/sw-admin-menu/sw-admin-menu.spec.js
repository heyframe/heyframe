...
const menuService = createMenuService(HeyFrame.Module);
HeyFrame.Service().register('menuService', () => menuService);

async function createWrapper(options = {}) {
    const router = createRouter({
        routes: [
            ...HeyFrame.Module.getModuleRoutes(),
            {
...
describe('src/app/component/structure/sw-admin-menu', () => {
    ...
    beforeAll(() => {
        ...
        HeyFrame.Context.app.fallbackLocale = 'en-GB';

        HeyFrame.Module.getModuleRegistry().clear();
        adminModules.forEach((adminModule) => {
            HeyFrame.Module.register(adminModule.name, adminModule);
        });
    });
    ...
    beforeEach(async () => {
        ...
        jest.spyOn(HeyFrame.Utils.debug, 'error').mockImplementation(() => true);

        HeyFrame.Store.get('session').setCurrentUser(null);
    ...
    it('should not render 4.level or higher menu item and throw error', async () => {
        ...
        expect(HeyFrame.Utils.debug.error.mock.calls[0][0]).toBeInstanceOf(Error);
        expect(HeyFrame.Utils.debug.error.mock.calls[0][0].toString()).toBe(
        ...
        expect(HeyFrame.Utils.debug.error.mock.calls[1][0]).toBeInstanceOf(Error);
        expect(HeyFrame.Utils.debug.error.mock.calls[1][0].toString()).toBe(
    ...
    describe('app menu entries', () => {
        it('renders apps under there parent navigation entry', async () => {
            HeyFrame.Store.get('heyframeApps').apps = testApps;
            await flushPromises();
