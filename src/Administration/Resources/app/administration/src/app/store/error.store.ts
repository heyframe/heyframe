 ...
 */
import type HeyFrameError from 'src/core/data/HeyFrameError';
 ...
 * */
function setApiErrorNested({ expression, error, store }: { expression: string; error: HeyFrameError; store: ErrorStore }) {
...
function removeApiError(expression: string, store: ErrorStore) {
    ...
    if (HeyFrame.Utils.types.isEmpty(store)) {
 ...
 */
const errorStore = HeyFrame.Store.register({
    id: 'error',
    ...
    state: () =>
        ({
            ...
            system: Record<string, HeyFrameError>;
            api: Record<string, HeyFrameError>;
        },

    actions: {
        addApiError({ expression, error }: { expression: string; error: HeyFrameError }) {
        ...
        addSystemError({ error, id = utils.createId() }: { error: HeyFrameError; id?: string }) {
    ...
    getters: {
        getErrorsForEntity(state) {
            return (entityName: string, id: string) => {
                ...
                // @ts-expect-error
                return (entityStorage[id] as HeyFrameError) ?? null;
            };
        },
