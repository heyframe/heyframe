...
const adminMenuStore = HeyFrame.Store.register({
    id: 'adminMenu',
    ...
    getters: {
        appModuleNavigation() {
            const menuService = HeyFrame.Service('menuService') as MenuService;
            // eslint-disable-next-line no-warning-comments
            // TODO: Change this when `heyframeApps` store is converted to Pinia
            const heyframeAppsState = HeyFrame.Store.get('heyframeApps') as { apps: AppModuleDefinition[] };

            return menuService?.getNavigationFromApps(heyframeAppsState.apps);
        },
    },
});
