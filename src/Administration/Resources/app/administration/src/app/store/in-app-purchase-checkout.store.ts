 ...
 * @sw-package checkout
 */
import type { iapCheckout } from '@heyframe-ag/meteor-admin-sdk/es/iap';
...
const inAppPurchaseCheckoutStore = HeyFrame.Store.register({
    ...
    actions: {
        request(entry: InAppPurchaseRequest, extension: string): void {
            if (!HeyFrame.Context.app.config.bundles?.[extension]) {
