...
import setupHeyFrameDevtools from 'src/app/adapter/view/sw-vue-devtools';
...
HeyFrame.Service().register('localeHelper', () => {
    return {
...
describe('ASYNC app/adapter/view/vue.adapter.js', () => {
    ...
    beforeEach(async () => {
        ...
        if (!HeyFrame.Service('loginService')) {
        ...
        if (!HeyFrame.Service('localeToLanguageService')) {
            HeyFrame.Service().register('localeToLanguageService', () => {
                return {
    ...
    it('initLocales should call setLocaleFromuser', async () => {
        ...
        HeyFrame.Store.get('session').currentLocale = 'en-GB';
    ...
    it('setLocaleFromUser should not set the user when user does not exist', async () => {
        ...
        expect(HeyFrame.Service('localeHelper').setLocaleWithId).not.toHaveBeenCalled();
    });

    it('setLocaleFromUser should set the user when user does not exist', async () => {
        HeyFrame.Store.get('session').setCurrentUser({ localeId: '12345' });
        vueAdapter.setLocaleFromUser();

        expect(HeyFrame.Service('localeHelper').setLocaleWithId).toHaveBeenCalled();
    });

    it('setLocaleFromUser should call the service with the user id from the store', async () => {
        const expectedId = '12345678';
        HeyFrame.Store.get('session').setCurrentUser({ localeId: expectedId });

        vueAdapter.setLocaleFromUser();

        expect(HeyFrame.Service('localeHelper').setLocaleWithId).toHaveBeenCalledWith(expectedId);
    });
    ...
    it('initLocales should watch for user changes and recall the "setLocaleWithId"', async () => {
        ...
        expect(HeyFrame.Service('localeHelper').setLocaleWithId).toHaveBeenCalledWith('german-id');
    });

    it('should resolve mixins by explicit Mixin get by name call', async () => {
        HeyFrame.Mixin.register('foo1', {
            methods: {
                fooBar() {
                    return this.title;
                },
        ...
        HeyFrame.Component.register('test-component1', {
            template: '<div></div>',
        ...
        HeyFrame.Component.markComponentAsSync('test-component1');
    ...
    it('should resolve mixins by explicit Mixin get by name call with override', async () => {
        ...
        HeyFrame.Component.register('test-component2', {
            template: '<div></div>',
        ...
        HeyFrame.Component.override('test-component2', {
            data() {
                return {
        ...
        HeyFrame.Component.markComponentAsSync('test-component2');
    ...
    it('should resolve mixins by string', async () => {
        ...
        HeyFrame.Component.register('test-component3', {
            template: '<div></div>',
        ...
        HeyFrame.Component.markComponentAsSync('test-component3');
    ...
    it('should resolve mixins by string with override', async () => {
        ...
        HeyFrame.Component.register('test-component4', {
            template: '<div></div>',
        ...
        HeyFrame.Component.override('test-component4', {
            data() {
                return {
        ...
        HeyFrame.Component.markComponentAsSync('test-component4');
    ...
    it('should resolve mixins for component in combination with overrides', async () => {
        HeyFrame.Mixin.register('foo-with-data', {
            data() {
                return {
                    sortBy: null,
                };
        ...
        HeyFrame.Component.register('test-component-foobar-with-mixin', {
            template: '<div></div>',
            name: 'test-component',
            data() {
                return {
        ...
        HeyFrame.Component.markComponentAsSync('test-component-foobar-with-mixin');
        ...
        HeyFrame.Component.override('test-component-foobar-with-mixin', {});
    ...
    it('should extend mixins', async () => {
        ...
        HeyFrame.Component.register('extendable-component', {
        ...
        HeyFrame.Component.extend('sw-test-component-extended', 'extendable-component', {
        ...
        HeyFrame.Component.markComponentAsSync('sw-test-component-extended');
    ...
    it('should allow multi-inheritance with multiple mixins and lifecycle hooks are only executed once', async () => {
        ...
        HeyFrame.Mixin.register('first-mixin', {
            created() {
                lifecycleSpy();
            },
            methods: {
        ...
        HeyFrame.Mixin.register('second-mixin', {
            methods: {
                bar() {
                    return 'bar';
                },
        ...
        HeyFrame.Component.register('base-component', {
            template: '<div class="base-component"></div>',
        });

        HeyFrame.Component.override('base-component', {
            mixins: ['first-mixin'],
        });
        ...
        HeyFrame.Component.markComponentAsSync('base-component');
    ...
    describe('should initialize everything correctly', () => {
        ...
        beforeEach(async () => {
            ...
            const locale = HeyFrame.Application.getContainer('factory').locale;
            if (!locale.getLocaleByName('en-GB')) {
                locale.register('en-GB', {
                    global: {
                        'sw-admin-menu': {
                            textHeyFrameAdmin: 'Text HeyFrame Admin',
                        },
                        my: {
                            mock: {
                                title: 'Mock title',
            ...
            if (!HeyFrame.Filter.getByName('my-mock-filter')) {
                HeyFrame.Filter.register('my-mock-filter', () => {
                    return 'mocked';
                });
            }

            if (!HeyFrame.Directive.getByName('my-mock-directive')) {
                HeyFrame.Directive.register('my-mock-directive', () => {
                    return {
                        bind() {},
                        inserted() {},
                        update() {},
            ...
            // add main component
            if (!HeyFrame.Component.getComponentRegistry().has('sw-admin')) {
                HeyFrame.Component.register('sw-admin', {
                    template: '<div class="sw-admin"></div>',
                });
            }

            // add VueAdapter to HeyFrame object
            HeyFrame.Application.setViewAdapter(vueAdapter);
        ...
        it('should have correct working createTitle method', () => {
            ...
            expect(result).toBe('Test | Mock title | Text HeyFrame Admin');
        });
        ...
        it('should setup the devtools in development environment', async () => {
            expect(setupHeyFrameDevtools).toHaveBeenCalled();
        });
