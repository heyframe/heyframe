...
export default class VueAdapter extends ViewAdapter {
    ...
    initVue(renderElement: string, router: Router, providers: { [key: string]: unknown }): App<Element> {
         ...
         *
         * So we should convert from provide/inject to HeyFrame.Service
         */
        ...
        if (process.env.NODE_ENV === 'development') {
            setupHeyFrameDevtools(this.root);
        }
    ...
    async initComponents() {
        ...
        meteorComponents.forEach((componentName) => {
            const componentNameAsKebabCase = HeyFrame.Utils.string.kebabCase(componentName);
    ...
    initLocales() {
        ...
        const messages = {};
        const fallbackLocale = HeyFrame.Context.app.fallbackLocale as FallbackLocale;
        ...
        HeyFrame.Vue.watch(
            ...
            (newValue, oldValue) => {
                ...
                if (currentUserLocaleId && currentUserLocaleId !== oldUserLocaleId) {
                    // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-member-access
                    HeyFrame.Service('localeHelper').setLocaleWithId(currentUserLocaleId);
                }
            },
            { deep: true },
        );
    ...
    setLocaleFromUser() {
        ...
        if (currentUser) {
            ...
            // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-member-access
            HeyFrame.Service('localeHelper').setLocaleWithId(userLocaleId);
        }
    }
    ...
    initTitle(app: App<Element>) {
        app.config.globalProperties.$createTitle = function createTitle(
            ...
            const baseTitle = this.$root.$tc('global.sw-admin-menu.textHeyFrameAdmin');
