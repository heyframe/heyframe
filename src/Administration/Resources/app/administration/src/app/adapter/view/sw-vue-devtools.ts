 ...
 * @private
 */
export default function setupHeyFrameDevtools(app: App): void {
    setupDevtoolsPlugin(
        {
            // Options
            id: 'sw-admin-extension-plugin',
            label: 'HeyFrame Admin extensions plugin',
        ...
        `;
            ...
            api.on.getInspectorTree((payload) => {
                ...
                const publishedDatasets = HeyFrame.ExtensionAPI.getPublishedDataSets();
            ...
            api.on.getInspectorState((payload) => {
                ...
                if (payload.nodeId.startsWith(DATASET_ID_PREFIX)) {
                    ...
                    const value =
                        HeyFrame.ExtensionAPI.getPublishedDataSets().find((set) => set.id === datasetId)?.data ?? 'unknown';
