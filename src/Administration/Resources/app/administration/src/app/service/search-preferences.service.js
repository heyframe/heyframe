...
export default function SearchPreferencesService({ userConfigRepository: _userConfigRepository }) {
    ...
    function getDefaultSearchPreferences() {
        ...
        HeyFrame.Module.getModuleRegistry().forEach(({ manifest }) => {
            if (
                manifest.entity &&
                HeyFrame.Service('acl').can(`${manifest.entity}.editor`) &&
    ...
    function getUserSearchPreferences() {
        return new Promise((resolve) => {
            HeyFrame.Service('userConfigService')
    ...
    function processSearchPreferencesFields(tempSearchPreferencesFields) {
        ...
        tempSearchPreferencesFields.forEach((field) => {
            field.group.forEach((group) => {
                const searchPreferencesField = HeyFrame.Utils.object.set({}, group.fieldName, {
                    _searchable: field._searchable,
                    _score: field._score,
                });
                searchPreferencesFields = HeyFrame.Utils.object.deepMergeObject(
    ...
    function _getUserConfigCriteria() {
        const criteria = new HeyFrame.Data.Criteria();

        criteria.addFilter(HeyFrame.Data.Criteria.equals('key', KEY_USER_SEARCH_PREFERENCE));
        criteria.addFilter(HeyFrame.Data.Criteria.equals('userId', _getCurrentUser()?.id));
