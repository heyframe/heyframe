...
export default class FilterService {
    ...
    getStoredFilters(storeKey) {
        ...
        return this._userConfigRepository.search(criteria, HeyFrame.Context.api).then((response) => {
            if (response.length) {
                this._filterEntity = response.first();
            } else {
                const currentUser = HeyFrame.Store.get('session').currentUser;

                this._filterEntity = this._userConfigRepository.create(HeyFrame.Context.api);
    ...
    saveFilters(storeKey, filters) {
        ...
        this._userConfigRepository.save(this._filterEntity, HeyFrame.Context.api).then(() => {
    ...
    async _pushFiltersToUrl(replaceRoute = false) {
        ...
        const router = HeyFrame.Application.view.router;
        ...
        if (!HeyFrame.Utils.types.isEmpty(routeParams)) {
