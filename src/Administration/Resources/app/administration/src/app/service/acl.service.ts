...
export default class AclService {
    isAdmin(): boolean {
        ...
        return !!HeyFrame.Store.get('session').currentUser?.admin;
    }
    ...
    can(privilegeKey: string): boolean {
        ...
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
        return (HeyFrame.Store.get('session').userPrivileges as string[]).includes(privilegeKey);
    }
    ...
    hasAccessToRoute(path: string): boolean {
        ...
        if (!HeyFrame?.Application?.view?.root?.$router) {
            return true;
        }

        const router = HeyFrame.Application.view.root.$router;
    ...
    hasActiveSettingModules(): boolean {
        ...
        const groups = Object.values(HeyFrame.Store.get('settingsItems').settingsGroups) as [[{ privilege?: string }]];
    ...
    get privileges(): string[] {
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
        return HeyFrame.Store.get('session').userPrivileges as string[];
    }
}
