const { Application, Store } = HeyFrame;
...
export default function createLicenseViolationsService(storeService) {
    ...
    async function forceDeletePlugin(extension) {
        const heyframeExtensionService = HeyFrame.Service('heyframeExtensionService');
        const cacheService = HeyFrame.Service('cacheApiService');
        ...
        try {
            ...
            if (isActive) {
                await heyframeExtensionService.deactivateExtension(extension.name, extension.type);
                await cacheService.clear();
            }

            if (isInstalled) {
                await heyframeExtensionService.uninstallExtension(extension.name, extension.type);
            }

            await heyframeExtensionService.removeExtension(extension.name, extension.type);
