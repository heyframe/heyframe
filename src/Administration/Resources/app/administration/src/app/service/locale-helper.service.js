...
export default class LocaleHelperService {
    ...
    constructor({ HeyFrame, localeRepository, snippetService, localeFactory }) {
        this._HeyFrame = HeyFrame;
    ...
    async setLocaleWithId(localeId) {
        const { code } = await this._localeRepository.get(localeId, this._HeyFrame.Context.api);
    ...
    async setLocaleWithCode(localeCode) {
        ...
        await this._HeyFrame.Store.get('session').setAdminLocale(localeCode);
    }
}
