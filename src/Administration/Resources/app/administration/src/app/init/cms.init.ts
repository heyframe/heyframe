...
export default function initializeCms(): void {
    HeyFrame.ExtensionAPI.handle('cmsRegisterElement', (element, additionalInformation) => {
    ...
    HeyFrame.ExtensionAPI.handle('cmsRegisterBlock', (block, additionalInformation) => {
        const extension = Object.values(HeyFrame.Store.get('extensions').extensionsState).find((ext) =>
            ext.baseUrl.startsWith(additionalInformation._event_.origin),
        );
