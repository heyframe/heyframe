...
export default function initializeNotifications(): void {
    ...
    HeyFrame.ExtensionAPI.handle('notificationDispatch', (notificationOptions) => {
        // @ts-expect-error - t is callable
        const message = notificationOptions.message ?? HeyFrame.Snippet.tc('global.notification.noMessage');
        // @ts-expect-error - tc is callable
        const title = notificationOptions.title ?? HeyFrame.Snippet.tc('global.notification.noTitle');
