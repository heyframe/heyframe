...
export default function initializeTabs(): void {
    HeyFrame.ExtensionAPI.handle('uiTabsAddTabItem', async (componentConfig) => {
        HeyFrame.Store.get('tabs').addTabItem(componentConfig);

        // Reload current route if it does not exist
        const router = HeyFrame.Application.view?.router as Router;
    ...
    void HeyFrame.Application.viewInitialized.then(() => {
        ...
        router.beforeResolve((to) => {
            ...
            // Get all tab routes
            const tabRoutes = Object.values(HeyFrame.Store.get('tabs').tabItems).reduce<string[]>((acc, tabItems) => {
                acc = [
                    ...acc,
            ...
            const moduleInfo = HeyFrame?.Application?.$container?.container?.init?.router?.getModuleInfo?.(parentRoute) as
                | {
            ...
            const newRoute: RouteRecordRaw = {
                path: newPath,
                component: HeyFrame.Application.view?.getComponent('sw-extension-component-section'),
                children: [],
                name: newRouteName,
                meta: {
