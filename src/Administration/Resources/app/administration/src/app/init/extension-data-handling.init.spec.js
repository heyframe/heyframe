...
import { send } from '@heyframe-ag/meteor-admin-sdk/es/channel';
...
describe('src/app/init/extension-data-handling.init.ts', () => {
    ...
    beforeAll(() => {
        jest.spyOn(HeyFrame.Service('repositoryFactory'), 'create').mockImplementation((entityName) => {
    ...
    beforeEach(() => {
        HeyFrame.Service('repositoryFactory').create.mockClear();
    ...
    it('should handle repositorySearch', async () => {
        ...
        expect(HeyFrame.Service('repositoryFactory').create).toHaveBeenCalledWith('product');
