...
async function createWrapper() {
    return mount(await wrapTestComponent('sw-navigation-detail-products', { sync: true }), {
        global: {
            stubs: {
                'router-link': true,
                'sw-container': true,
...
describe('module/sw-navigation/view/sw-navigation-detail-products.spec', () => {
    beforeEach(async () => {
    ...
    it('should render stream select when changing the assignment type to stream', async () => {
        ...
        await wrapper
            .getComponent('.sw-navigation-detail-products__product-assignment-type-select')
        ...
        expect(wrapper.find('.sw-navigation-detail-products__product-stream-select').exists()).toBe(true);
    });
    ...
    it('should show message when assignment type is product stream and products are manually assigned', async () => {
        ...
        expect(wrapper.find('[role="banner"]').text()).toBe(
            'sw-navigation.base.products.alertManualAssignedProductsOnAssignmentTypeStream',
        );
    });
    ...
    it('should try to load product stream preview when stream id is present', async () => {
        ...
        await wrapper
            .getComponent('.sw-navigation-detail-products__product-stream-select')
