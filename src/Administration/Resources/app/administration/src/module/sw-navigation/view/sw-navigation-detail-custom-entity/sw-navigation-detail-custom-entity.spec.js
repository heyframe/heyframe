...
async function createWrapper() {
    ...
    HeyFrame.Store.get('swCategoryDetail').navigation = {
        ...
        extensions: {
            customEntityName1SwNavigations: customEntity1.instanceRepository,
            customEntityName2SwNavigations: customEntity2.instanceRepository,
        },
    };

    return mount(
        await wrapTestComponent('sw-navigation-detail-custom-entity', {
            sync: true,
        }),
        {
            global: {
...
describe('src/module/sw-navigation/view/sw-navigation-detail-custom-entity/index.ts', () => {
    it('should allow selecting a custom entity', async () => {
        ...
        expect(wrapper.getComponent('.sw-navigation-detail-custom-entity__selection-container').props()).toEqual({
            positionIdentifier: 'navigation-detail-custom-entity',
            title: 'sw-navigation.base.customEntity.cardTitle',
        });

        const entitySelect = wrapper.getComponent('.sw-entity-single-select');
        expect(entitySelect.props()).toEqual({
            value: undefined,
            label: 'sw-navigation.base.customEntity.assignment.label',
            helpText: 'sw-navigation.base.customEntity.assignment.helpText',
            disabled: false,
        ...
        expect(wrapper.vm.navigation.customEntityTypeId).toBe(customEntity1.id);
        expect(wrapper.vm.customEntityAssignments).toStrictEqual(customEntity1.instanceRepository);

        expect(wrapper.find('.sw-navigation-detail-custom-entity__selection-container').exists()).toBe(false);
        expect(wrapper.getComponent('.sw-many-to-many-assignment-card').props()).toEqual({
            columns: [
                {
                    dataIndex: 'cmsAwareTitle',
                    label: 'sw-navigation.base.customEntity.instanceAssignment.title',
            ...
            localMode: false,
            placeholder: 'sw-navigation.base.customEntity.instanceAssignment.placeholder',
            selectLabel: 'sw-navigation.base.customEntity.instanceAssignment.label',
            title: 'sw-navigation.base.customEntity.cardTitle',
        });
    });
