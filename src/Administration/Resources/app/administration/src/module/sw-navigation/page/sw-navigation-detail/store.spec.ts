...
const categoriesMock: Record<string, Partial<EntitySchema.Entities['navigation']>> = {
    'without-parent': {
        id: '12345',
    },
    'with-parent': {
    ...
    parent: {
        id: '111213',
        footerChannels: [{ typeId: '12345' }] as EntitySchema.EntityCollection<'sales_channel'>,
    },
};
...
} as unknown as Repository<'navigation'>;

describe('sw-navigation.store', () => {
    beforeEach(() => {
        setActivePinia(createPinia());
    });

    it('should have the initial state', () => {
        const swCategoryDetailStore = HeyFrame.Store.get('swCategoryDetail');

        expect(swCategoryDetailStore.landingPage).toBeNull();
        expect(swCategoryDetailStore.navigation).toBeNull();
    ...
    it('loads an active navigation', async () => {
        const swCategoryDetailStore = HeyFrame.Store.get('swCategoryDetail');

        await swCategoryDetailStore.loadActiveCategory({
            id: 'without-parent',
            repository: navigationRepositoryMock,
            apiContext: apiContextMock,
        });

        // eslint-disable-next-line @typescript-eslint/unbound-method
        expect(navigationRepositoryMock.get).toHaveBeenCalledWith('without-parent', apiContextMock, expect.anything());
        expect(swCategoryDetailStore.isCategoryColumn).toBe(false);
        expect(swCategoryDetailStore.navigation).toStrictEqual(categoriesMock['without-parent']);
    });

    it('loads an active navigation with parent', async () => {
        const swCategoryDetailStore = HeyFrame.Store.get('swCategoryDetail');
        ...
        // eslint-disable-next-line @typescript-eslint/unbound-method
        expect(navigationRepositoryMock.get).toHaveBeenCalledWith('with-parent', apiContextMock, expect.anything());
        expect(swCategoryDetailStore.isCategoryColumn).toBe(true);
        expect(swCategoryDetailStore.navigation).toStrictEqual({
