import template from './sw-navigation-detail-products.html.twig';
import './sw-navigation-detail-products.scss';
...
export default {
    ...
    computed: {
        ...
        productColumns() {
            return [
                {
                    property: 'name',
                    label: this.$tc('sw-navigation.base.products.columnNameLabel'),
                    dataIndex: 'name',
                ...
                },
                {
                    ...
                    label: this.$tc('sw-navigation.base.products.columnManufacturerLabel'),
        ...
        productStreamInvalidError() {
            if (this.productStreamInvalid) {
                return new HeyFrameError({
                    ...
                    detail: this.$tc('sw-navigation.base.products.dynamicProductGroupInvalidMessage'),
                });
            }
            return null;
        },
        ...
        productAssignmentTypes() {
            return [
                {
                    value: 'product',
                    label: this.$tc('sw-navigation.base.products.productAssignmentTypeManualLabel'),
                },
                {
                    value: 'product_stream',
                    label: this.$tc('sw-navigation.base.products.productAssignmentTypeStreamLabel'),
                },
            ];
        },
        ...
        dynamicProductGroupHelpText() {
            ...
            const helpText = this.$tc(
                'sw-navigation.base.products.dynamicProductGroupHelpText.label',
                {
                    link: `<sw-internal-link
                           :router-link=${JSON.stringify(link)}
                           :inline="true">
                           ${this.$tc('sw-navigation.base.products.dynamicProductGroupHelpText.linkText')}
                       </sw-internal-link>`,
                },
                0,
            );
            ...
            try {
                ...
                new URL(this.$tc('sw-navigation.base.products.dynamicProductGroupHelpText.videoUrl'));
            } catch {
                return helpText;
            }
            ...
            return `${helpText}
                    <br>
                    <sw-external-link
                        href="${this.$tc('sw-navigation.base.products.dynamicProductGroupHelpText.videoUrl')}">
                        ${this.$tc('sw-navigation.base.products.dynamicProductGroupHelpText.videoLink')}
                    </sw-external-link>`;
        },
    ...
    methods: {
        createdComponent() {
            if (!this.navigation.productStreamId) {
                return;
            }
            this.loadProductStreamPreview();
        },
        ...
        loadProductStreamPreview() {
            this.productStreamRepository
                .get(this.navigation.productStreamId)
                .then((response) => {
