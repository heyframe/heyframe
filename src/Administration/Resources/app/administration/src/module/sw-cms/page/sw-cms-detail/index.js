...
const { Component, Mixin, Utils } = HeyFrame;
const { mapPropertyErrors } = Component.getComponentHelper();
const { HeyFrameError } = HeyFrame.Classes;
const { debounce } = HeyFrame.Utils;
const { cloneDeep, getObjectDiff } = HeyFrame.Utils.object;
const { isEmpty } = HeyFrame.Utils.types;
const { warn } = HeyFrame.Utils.debug;
const { Criteria } = HeyFrame.Data;
const { CMS } = HeyFrame.Constants;
...
export default {
    ...
    methods: {
        ...
        async loadPage(pageId) {
            ...
            try {
                this.page = await this.pageRepository.get(pageId, HeyFrame.Context.api, this.loadPageCriteria);
        ...
        async loadDemoProduct(entityId) {
            ...
            const context = { ...HeyFrame.Context.api, inheritance: true };
        ...
        onSaveEntity() {
            ...
            return this.pageRepository
                .save(this.page, HeyFrame.Context.api, false)
                .then(() => {
                    this.isLoading = false;
        ...
        addError({ property = null, payload = {}, code = CMS.REQUIRED_FIELD_ERROR_CODE, message = '' } = {}) {
            ...
            const error = new HeyFrameError({
                code,
                detail: message,
                meta: { parameters: payload },
            });

            HeyFrame.Store.get('error').addApiError({ expression, error });
        },

        getError(property) {
            return HeyFrame.Store.get('error').getApiError(this.page, property);
        },
        ...
        async setDefaultLayout() {
            ...
            const isLiveVersion = this.page.versionId === HeyFrame.Context.api.liveVersionId;
