...
import type Criteria from '@heyframe-ag/meteor-admin-sdk/es/data/Criteria';
...
class CmsService {
    ...
    public registerCmsElement(config: CmsElementConfig): boolean {
        ...
        HeyFrame.Application.view?.setReactive(this.elementRegistry, config.name, config);
    ...
    public getEntityMappingTypes(entityName: string) {
        const schema = HeyFrame.EntityDefinition.has(entityName) ? HeyFrame.EntityDefinition.get(entityName) : undefined;
    ...
    private async addCustomFieldsToMappingTypes(entityName: string, mappings: EntityMappings): Promise<void> {
        const customFieldRepository = HeyFrame.Service('repositoryFactory').create('custom_field');
        const criteria = new HeyFrame.Data.Criteria(1, 50);
        criteria.addFilter(HeyFrame.Data.Criteria.equals('customFieldSet.relations.entityName', entityName));
        criteria.addFilter(HeyFrame.Data.Criteria.equals('active', 1));
        criteria.addFilter(
            HeyFrame.Data.Criteria.multi('OR', [
                HeyFrame.Data.Criteria.equals('type', 'text'),
                HeyFrame.Data.Criteria.equals('type', 'datetime'),
                HeyFrame.Data.Criteria.equals('type', 'html'),
            ]),
        );

        const customFields = await customFieldRepository.search(criteria, HeyFrame.Context.api);
    ...
    private handleAssociationMapping(
        ...
        if (toOneAssociation && propSchema.entity) {
            ...
            if (deep) {
                const schema = HeyFrame.EntityDefinition.get(propSchema.entity);
    ...
    static getEntityData(slot: RuntimeSlot, configKey: string): CmsSlotData {
        ...
        const entityData: CmsSlotData = {
            value: [],
            key: configKey,
            searchCriteria: new HeyFrame.Data.Criteria(1, 25),
            ...entity,
        };
        ...
        entityData.searchCriteria = entity?.criteria ? entity.criteria : new HeyFrame.Data.Criteria(1, 25);
    ...
    Object.keys(slot.config).forEach((configKey) => {
        ...
        if (entity && value) {
            ...
            const entityData: CmsSlotData = {
                value: [value].flat(),
                key: configKey,
                searchCriteria: entity.criteria ? entity.criteria : new HeyFrame.Data.Criteria(1, 25),
                ...entity,
            };
