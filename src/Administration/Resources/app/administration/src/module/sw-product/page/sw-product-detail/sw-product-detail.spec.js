...
describe('module/sw-product/page/sw-product-detail', () => {
    ...
    beforeEach(async () => {
        ...
        HeyFrame.Store.get('swProductDetail').setLengthUnit = jest.fn();
        HeyFrame.Store.get('swProductDetail').setWeightUnit = jest.fn();
    });
    ...
    it('should show advanced mode settings', async () => {
        HeyFrame.Store.get('swProductDetail').product = { parentId: '' };
        await nextTick();
    ...
    it('should show item tabs when advanced mode deactivate', async () => {
        ...
        HeyFrame.Store.get('swProductDetail').advancedModeSetting = {
            value: {
    ...
    it('should run custom validation service and handle errors', async () => {
        ...
        HeyFrame.Store.get('swProductDetail').creationStates = 'is-download';
    ...
    it('should initialize with default units when no preferences exist', async () => {
        ...
        expect(HeyFrame.Store.get('swProductDetail').setLengthUnit).toHaveBeenCalledWith('mm');
        expect(HeyFrame.Store.get('swProductDetail').setWeightUnit).toHaveBeenCalledWith('kg');
    });
    ...
    it('should initialize with preferred units when they exist', async () => {
        ...
        expect(HeyFrame.Store.get('swProductDetail').setLengthUnit).toHaveBeenCalledWith('cm');
        expect(HeyFrame.Store.get('swProductDetail').setWeightUnit).toHaveBeenCalledWith('g');
    });
    ...
    it('should handle success response correctly', async () => {
        ...
        HeyFrame.Store.get('swProductDetail').setLoading = jest.fn();
        HeyFrame.Store.get('error').resetApiErrors = jest.fn();
        wrapper.vm.loadProduct = jest.fn();

        HeyFrame.Utils.EventBus.emit = jest.fn();

        wrapper.vm.onSaveFinished('success');

        expect(HeyFrame.Store.get('swProductDetail').setLoading).toHaveBeenCalledWith([
            'product',
            true,
        ]);
        ...
        expect(HeyFrame.Utils.EventBus.emit).toHaveBeenCalledWith('sw-product-detail-save-finish');
        expect(wrapper.vm.isSaveSuccessful).toBe(true);
        expect(HeyFrame.Store.get('error').resetApiErrors).not.toHaveBeenCalled();
        expect(HeyFrame.Store.get('swProductDetail').setLoading).toHaveBeenCalledWith([
            'product',
            false,
        ]);
    ...
    it('should not validate fields when language is inherited', async () => {
        ...
        HeyFrame.Store.get('context').api.language = {
            id: '1a2b3c',
            parentId: 'd4e5f6',
        };
