...
const { Context, Mixin, EntityDefinition } = HeyFrame;
const { Criteria, ChangesetGenerator } = HeyFrame.Data;
const { cloneDeep } = HeyFrame.Utils.object;
...
export default {
    ...
    computed: {
        ...
        localMode() {
            return HeyFrame.Store.get('swProductDetail').localMode;
        },

        advancedModeSetting() {
            return HeyFrame.Store.get('swProductDetail').advancedModeSetting;
        },

        modeSettings() {
            return HeyFrame.Store.get('swProductDetail').modeSettings;
        },
        ...
        advanceModeEnabled() {
            return HeyFrame.Store.get('swProductDetail').advanceModeEnabled;
        },
        ...
        language() {
            return HeyFrame.Store.get('context').api.language;
        },
    ...
    methods: {
        ...
        initState() {
            HeyFrame.Store.get('swProductDetail').apiContext = HeyFrame.Context.api;
        ...
        initAdvancedModeSettings() {
            HeyFrame.Store.get('swProductDetail').advancedModeSetting = this.getAdvancedModeDefaultSetting();
        ...
        getAdvancedModeSetting() {
            return this.userModeSettingsRepository.search(this.userModeSettingsCriteria).then(async (items) => {
                ...
                HeyFrame.Store.get('swProductDetail').advancedModeSetting = modeSettings;
                HeyFrame.Store.get('swProductDetail').modeSettings = this.changeModeSettings();
        ...
        onChangeSetting() {
            HeyFrame.Store.get('swProductDetail').advancedModeSetting = this.advancedModeSetting;
        ...
        loadState() {
            HeyFrame.Store.get('swProductDetail').localMode = false;
            HeyFrame.Store.get('heyframeApps').selectedIds = [
                this.productId,
            ];
        ...
        createState() {
            // set local mode
            HeyFrame.Store.get('swProductDetail').localMode = true;
            HeyFrame.Store.get('heyframeApps').selectedIds = [];
            ...
            // set product "type"
            HeyFrame.Store.get('swProductDetail').creationStates = this.creationStates;

            // create empty product
            HeyFrame.Store.get('swProductDetail').product = this.productRepository.create();
        ...
        loadProduct() {
            ...
            return this.productRepository
                ...
                .then(async (product) => {
                    ...
                    HeyFrame.Store.get('swProductDetail').product = product;

                    if (this.product.parentId) {
                        this.loadParentProduct();
                    } else {
                        HeyFrame.Store.get('swProductDetail').parentProduct = {};
                    }
        ...
        loadParentProduct() {
            ...
            return this.productRepository
                .get(this.product.parentId, HeyFrame.Context.api, this.productCriteria)
                .then(async (parent) => {
                    ...
                    HeyFrame.Store.get('swProductDetail').parentProduct = parent;
                })
                .then(() => {
                    HeyFrame.Store.get('swProductDetail').setLoading([
                        'parentProduct',
        ...
        loadCurrencies() {
            ...
            return this.currencyRepository
                ...
                .then((res) => {
                    HeyFrame.Store.get('swProductDetail').currencies = res;
                })
                .finally(() => {
                    HeyFrame.Store.get('swProductDetail').setLoading([
                        'currencies',
        ...
        loadAttributeSet() {
            ...
            return this.customFieldSetRepository
                ...
                .then((res) => {
                    HeyFrame.Store.get('swProductDetail').customFieldSets = res;
                })
                .finally(() => {
                    HeyFrame.Store.get('swProductDetail').setLoading([
                        'customFieldSets',
        ...
        onChangeLanguage(languageId) {
            HeyFrame.Store.get('context').setApiLanguageId(languageId);
        ...
        onSaveFinished(response) {
            ...
            Promise.all(this.updateSeoPromises)
                .then(() => {
                    HeyFrame.Utils.EventBus.emit('sw-product-detail-save-finish');
                })
                .then(() => {
                    switch (response) {
                        case 'empty': {
                            this.isSaveSuccessful = true;
                            HeyFrame.Store.get('error').resetApiErrors();
                            break;
                        }
        ...
        saveProduct() {
            ...
            if (HeyFrame.Store.list().includes('swSeoUrl')) {
                ...
                const defaultSeoUrl = HeyFrame.Store.get('swSeoUrl').defaultSeoUrl;
        ...
        async loadLanguage(newLanguageId) {
            HeyFrame.Store.get('context').api.language = await this.languageRepository.get(newLanguageId, {
                ...HeyFrame.Context.api,
                inheritance: true,
            });
        },
