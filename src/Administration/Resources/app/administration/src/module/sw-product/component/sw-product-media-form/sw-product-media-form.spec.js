                        ...
                        get: () => {
                            if (entity === 'media') {
                                return Promise.resolve({
                                    id: 'media1',
                                    url: 'http://heyframe.test/media1-new-url.jpg',
                                });
                            }
...
function getMediaCollection(collection = []) {
    return new EntityCollection('/media', 'media', null, { isHeyFrameContext: true }, collection, collection.length, null);
}
...
describe('module/sw-product/component/sw-product-media-form', () => {
    ...
    it('should remove previous mediaItem if it already exists after upload', async () => {
        ...
        expect(wrapper.vm.product.media[1].media.url).toBe('http://heyframe.test/media1-new-url.jpg');
    });
});
