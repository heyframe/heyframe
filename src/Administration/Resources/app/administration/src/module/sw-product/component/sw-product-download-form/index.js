...
export default {
    ...
    computed: {
        ...
        error() {
            return HeyFrame.Store.get('error').getApiError(this.product, 'downloads');
        },
    ...
    methods: {
        ...
        async successfulUpload({ targetId }) {
            ...
            if (this.error) {
                HeyFrame.Store.get('error').removeApiError(this.error.selfLink);
            }
        },
