...
const { Component, Mixin, Store } = HeyFrame;
    ...
    methods: {
        ...
        loadLanguages() {
            ...
            return this.languageRepository.search(languageCriteria).then((result) => {
                ...
                result.forEach((lang) => {
                    ...
                    if (lang.locale.code === HeyFrame.Context.app.fallbackLocale) {
            ...
            if (!this.acl.can('user:editor')) {
                ...
                this.userService
                    ...
                    .then(async () => {
                        ...
                        HeyFrame.Service('localeHelper').setLocaleWithId(this.user.localeId);
                    })
                    .catch((error) => {
                        HeyFrame.Store.get('error').addApiError({
                            expression: `user.${this.user?.id}.password`,
                            error: new HeyFrame.Classes.HeyFrameError(error.response.data.errors[0]),
                        });
                        this.createNotificationError({
        ...
        updateCurrentUser() {
            return this.userService.getUser().then((response) => {
                ...
                return HeyFrame.Store.get('session').setCurrentUser(data);
            });
        },
