...
import type { PermissionsConsent, ServiceConfiguration } from '../store/heyframe-services.store';
 ...
 * @private
 */
export default class HeyFrameServicesService extends ApiService {
    constructor(
        ...
        this.name = 'HeyFrameServices';
    }

    getInstalledServices(): Promise<ServiceDescription[]> {
        let languageId = HeyFrame.Store.get('session').languageId;
        if (!languageId) {
