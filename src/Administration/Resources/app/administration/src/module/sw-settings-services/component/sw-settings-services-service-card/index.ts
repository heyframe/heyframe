...
import { MtPopoverItem, MtModalRoot, MtModal, MtModalAction } from '@heyframe-ag/meteor-component-library';
import type { CategorizedPermissions, ServiceDescription } from '../../service/heyframe-services.service';
...
export default HeyFrame.Component.wrapComponentConfig({
    ...
    methods: {
        ...
        async setActive(active: boolean, toggleFloatingUi?: () => void) {
            ...
            try {
                const extensionService = HeyFrame.Service('heyframeExtensionService');
        ...
        async openPermissionsModal(toggleFloatingUi: () => void) {
            try {
                if (this.categorizedPermissions === null) {
                    const servicesService = HeyFrame.Service('heyframeServicesService');
