...
describe('src/module/sw-settings-services/component/sw-settings-services-service-card.ts', () => {
    ...
    beforeAll(() => {
        HeyFrame.Service().register('heyframeExtensionService', () => ({
    ...
    it('opens the deactivation modal and deactivates a service', async () => {
        HeyFrame.Service('heyframeExtensionService').deactivateExtension.mockImplementationOnce(() => {
        ...
        expect(HeyFrame.Service('heyframeExtensionService').deactivateExtension).toHaveBeenCalledWith('service-name', 'app');
        expect(window.location.reload).toHaveBeenCalled();
    });

    it('activates a service', async () => {
        HeyFrame.Service('heyframeExtensionService').activateExtension.mockImplementationOnce(() => {
            return Promise.resolve();
        });
        ...
        expect(HeyFrame.Service('heyframeExtensionService').activateExtension).toHaveBeenCalledWith('service-name', 'app');
        expect(window.location.reload).toHaveBeenCalled();
    });

    it('shows permissions modal for a service', async () => {
        HeyFrame.Service('heyframeServicesService').getCategorizedPermissions.mockImplementationOnce(async () => ({
            permissions: {
                order: [
                    {
                        extensions: [],
