...
class BulkEditCustomerHandler extends BulkEditBaseHandler {
    constructor() {
        ...
        this.entityIds = [];
        this.customerGroupRegistrationService = HeyFrame.Service('customerGroupRegistrationService');
        this.customerRepository = HeyFrame.Service('repositoryFactory').create('customer');
    }
    ...
    async bulkEdit(entityIds, payload) {
        ...
        return RetryHelper.retry(() => {
            return this.syncService.sync(
                ...
                {
                    'single-operation': 1,
                    'sw-language-id': HeyFrame.Context.api.languageId,
                },
            );
        });
    }
    ...
    async bulkEditRequestedGroup(entityIds, payload) {
        const promises = [];
        const shouldTriggerFlows = HeyFrame.Store.get('swBulkEdit').isFlowTriggered;
