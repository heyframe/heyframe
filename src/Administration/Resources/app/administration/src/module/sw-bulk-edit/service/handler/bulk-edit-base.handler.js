...
const { getObjectDiff } = HeyFrame.Utils.object;
...
class BulkEditBaseHandler {
    constructor() {
        this.syncService = HeyFrame.Service('syncService');
        this.repositoryFactory = HeyFrame.Service('repositoryFactory');
    ...
    async buildBulkSyncPayload(changes) {
        const definition = HeyFrame.EntityDefinition.get(this.entityName);
        ...
        await Promise.all(
            changes.map(async (change) => {
                ...
                if (!field) {
                    HeyFrame.Utils.debug.warn('Entity factory', `Property ${this.entityName}.${change.field} not found`);
