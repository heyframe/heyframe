...
const { intersectionBy, chunk, uniqBy } = HeyFrame.Utils.array;
...
export default {
    ...
    methods: {
        async createdComponent() {
            ...
            this.order = this.orderRepository.create(HeyFrame.Context.api);
            ...
            const requests = payloadChunks.map((ids) => {
                ...
                criteria.addFilter(
                    Criteria.multi('AND', [
                        ...
                        Criteria.equals(versionField, HeyFrame.Context.api.liveVersionId),
                    ]),
                );
        ...
        fetchToStateMachineTransitions(states) {
            ...
            return this.stateMachineStateRepository
                .search(this.toStateMachineStatesCriteria(states), HeyFrame.Context.api)
                .then((response) => {
                    if (!response.length) {
                        return [];
                    }
