                    ...
                    repositoryFactory: {
                        create: (entityName) => {
                            ...
                            if (entityName === 'language') {
                                return {
                                    search: () =>
                                        Promise.resolve(new EntityCollection('', '', HeyFrame.Context.api, null, [], 0)),
                                    get: () => Promise.resolve(),
                                };
                            }
...
describe('modules/sw-users-permissions/page/sw-users-permissions-user-detail', () => {
    beforeAll(() => {
        HeyFrame.Service().register('timezoneService', () => {
    ...
    beforeEach(async () => {
        HeyFrame.Store.get('session').languageId = '123456789';
    ...
    afterEach(async () => {
        ...
        HeyFrame.Store.get('session').languageId = '';
    });
    ...
    it('should contain all fields', async () => {
        ...
        expect(fieldEmail.props('modelValue')).toBe('info@heyframe.com');
    ...
    it('should publish current user and editing user', async () => {
        expect(HeyFrame.ExtensionAPI.publishData).toHaveBeenCalledTimes(2);

        expect(HeyFrame.ExtensionAPI.publishData).toHaveBeenNthCalledWith(1, {
            id: 'sw-users-permissions-user-detail__currentUser',
            path: 'currentUser',
            scope: expect.anything(),
        });

        expect(HeyFrame.ExtensionAPI.publishData).toHaveBeenNthCalledWith(2, {
