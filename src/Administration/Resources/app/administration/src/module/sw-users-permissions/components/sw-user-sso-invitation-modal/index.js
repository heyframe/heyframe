...
export default {
    ...
    computed: {
        invitationService() {
            return HeyFrame.Service('ssoInvitationService');
        },

        languageRepository() {
            return HeyFrame.Service('repositoryFactory').create('language');
        },

        languageCriteria() {
            const registeredLocales = Array.from(HeyFrame.Locale.getLocaleRegistry().keys());
    ...
    methods: {
        ...
        async loadLanguages() {
            this.languages = await this.languageRepository.search(this.languageCriteria, HeyFrame.Context.api);
        },
        ...
        validateEmail() {
            if (!/[\w.%+-]+@[\w.-]+\.[\w]{2,}/.test(this.email)) {
                this.errors.errorEmail = new HeyFrameError({
        ...
        validateLanguage() {
            if (this.languageId === null) {
                this.errors.errorLanguage = new HeyFrameError({
