...
const { Defaults, EntityDefinition } = HeyFrame;
...
export default {
    ...
    computed: {
        ...
        isBusinessAccountType() {
            return this.customer?.accountType === HeyFrame.Constants.CUSTOMER.ACCOUNT_TYPE_BUSINESS;
        },
    },
    ...
    watch: {
        ...
        'country.forceStateInRegistration'(newVal) {
            if (!newVal) {
                HeyFrame.Store.get('error').removeApiError(
        ...
        'country.postalCodeRequired'(newVal) {
            if (!newVal) {
                HeyFrame.Store.get('error').removeApiError(`${this.address.getEntityName()}.${this.address.id}.zipcode`);
            }
