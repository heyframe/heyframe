...
function getCollection(repository, entities = [], aggregations = defaultAggregations, total = 0) {
    return new EntityCollection(
        ...
        repository,
        Context.api,
        { isHeyFrameContext: true, page: 1, limit: 25 },
        entities,
        total,
        aggregations,
    );
...
describe('src/module/sw-settings-rule/page/sw-settings-rule-detail', () => {
    ...
    it.each([
        ...
        const apiLanguageId = HeyFrame.Store.get('context').api.languageId;
        expect(HeyFrame.Store.get('context').api.languageId).not.toBe('uuid1');
        ...
        expect(HeyFrame.Store.get('context').api.languageId).toBe(abort ? apiLanguageId : 'uuid1');

        // cleanup
        HeyFrame.Store.get('context').api.languageId = apiLanguageId;
    });
