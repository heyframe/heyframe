...
export default {
    ...
    computed: {
        currentUserId() {
            return HeyFrame.Store.get('session').currentUser.id;
        },
    ...
    methods: {
        ...
        loadUserConfig() {
            return this.userConfigRepository.search(this.userConfigCriteria, HeyFrame.Context.api).then((userConfigs) => {
                if (userConfigs.length === 0) {
                    this.userConfig = this.userConfigRepository.create(HeyFrame.Context.api);
        ...
        onSave() {
            ...
            return this.countryRepository
                .save(this.country, HeyFrame.Context.api)
                .then(() => {
                    if (userConfigValue && Object.keys(userConfigValue).length > 0) {
                        this.userConfigRepository.save(this.userConfig, HeyFrame.Context.api).then(() => {
                            this.loadUserConfig();
                        });
                    }
                    this.loadEntityData();
        ...
        onUpdateCountry(path, value) {
            HeyFrame.Utils.object.set(this.country, path, value);
        },
    },
};
