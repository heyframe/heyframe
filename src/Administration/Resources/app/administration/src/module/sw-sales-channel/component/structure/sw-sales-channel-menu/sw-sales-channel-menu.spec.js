...
const headlessSalesChannel = {
    ...
    domains: [],
    type: {
        id: HeyFrame.Defaults.apiSalesChannelTypeId,
...
const storeFrontWithStandardDomain = {
    ...
    domains: [
        ...
        {
            languageId: HeyFrame.Defaults.systemLanguageId,
            url: 'http://shop/default-language',
        },
    ],
    type: {
        id: HeyFrame.Defaults.storefrontSalesChannelTypeId,
...
HeyFrame.Application.addServiceProvider('salesChannelFavorites', () => {
    const favorites = [];
...
describe('src/module/sw-sales-channel/component/structure/sw-sales-channel-menu', () => {
    beforeEach(async () => {
        HeyFrame.Service('salesChannelFavorites').state.favorites = [];
        HeyFrame.Store.get('session').languageId = defaultAdminLanguageId;
    ...
    it('shows "more" when more than 50 sales channels are available and marked as favourites', async () => {
        ...
        HeyFrame.Service('salesChannelFavorites').state.favorites = salesChannels.map((el) => el.id);
    ...
    it.each([
        ...
        HeyFrame.Utils.EventBus.emit(eventName);
        await flushPromises();
