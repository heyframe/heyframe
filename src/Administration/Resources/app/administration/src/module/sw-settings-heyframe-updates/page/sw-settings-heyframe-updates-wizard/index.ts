import template from './sw-settings-heyframe-updates-wizard.html.twig';
import './sw-settings-heyframe-updates-wizard.scss';
...
export default Component.wrapComponentConfig({
    ...
    computed: {
        ...
        updateButtonTooltip() {
            ...
            return {
                message: this.$tc('sw-settings-heyframe-updates.infos.requirementsNotMet'),
                position: 'bottom',
            };
        },
        ...
        optionDeactivateIncompatibleTranslation() {
            const deactivateIncompatTrans = this.$tc('sw-settings-heyframe-updates.plugins.actions.deactivateIncompatible');
            const isRecommended =
                this.displayIncompatiblePluginsWarning && !this.displayUnknownPluginsWarning
                    ? this.$tc('sw-settings-heyframe-updates.plugins.actions.recommended')
                    : '';
        ...
        optionDeactivateAllTranslation() {
            const deactiveAllTrans = this.$tc('sw-settings-heyframe-updates.plugins.actions.deactivateAll');
            const isRecommended =
                this.displayIncompatiblePluginsWarning && this.displayUnknownPluginsWarning
                    ? this.$tc('sw-settings-heyframe-updates.plugins.actions.recommended')
                    : '';
    ...
    methods: {
        ...
        startUpdateProcess() {
            ...
            this.createNotificationSuccess({
                message: this.$tc('sw-settings-heyframe-updates.notifications.updateStarted'),
            });
        ...
        stopUpdateProcess() {
            ...
            this.createNotificationInfo({
                message: this.$tc('sw-settings-heyframe-updates.notifications.updateStopped'),
            });
        },
            ...
            this.updateService
                ...
                .catch(() => {
                    this.createNotificationError({
                        message: this.$tc('sw-settings-heyframe-updates.notifications.downloadFailed'),
                    });
                });
        },
            ...
            this.updateService
                ...
                .then((response) => {
                    ...
                    if (response.offset === response.total) {
                        this.redirectToPage(`${HeyFrame.Context.api.basePath}/heyframe-installer.phar.php`);
                    } else {
                        this.deactivatePlugins(response.offset);
                    }
                })
                .catch((e: HeyFrameApiError) => {
                    ...
                    if (context.code === 'FRAMEWORK__PLUGIN_HAS_DEPENDANTS') {
                        ...
                        this.createNotificationError({
                            message: this.$tc('sw-settings-heyframe-updates.notifications.deactivationFailed'),
                        });
                    }
                });
        },
