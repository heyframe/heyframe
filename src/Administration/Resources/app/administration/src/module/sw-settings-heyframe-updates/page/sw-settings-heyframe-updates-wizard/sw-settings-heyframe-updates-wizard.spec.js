...
describe('module/sw-settings-heyframe-updates/page/sw-settings-heyframe-updates-wizard', () => {
    let wrapper;

    beforeEach(async () => {
        HeyFrame.Application.view.deleteReactive = () => {};
        wrapper = mount(
            await wrapTestComponent('sw-settings-heyframe-updates-wizard', {
                sync: true,
            }),
            {
                global: {
                        ...
                        updateService: {
                            ...
                            checkRequirements: () =>
                                Promise.resolve([
                                    ...
                                    {
                                        name: 'validHeyFrameLicense',
                                        result: false,
                                        message: 'validHeyFrameLicense',
                                        vars: [],
                                        extensions: [],
                                    },
                                ]),
                    ...
                    stubs: {
                        ...
                        'sw-settings-heyframe-updates-requirements': await wrapTestComponent(
                            'sw-settings-heyframe-updates-requirements',
                        ),
                        'sw-data-grid': await wrapTestComponent('sw-data-grid'),
                        'sw-card-view': await wrapTestComponent('sw-card-view'),
                        'sw-ignore-class': true,
                        'sw-settings-heyframe-updates-info': {
                            template: '<div></div>',
                        },
                        'sw-settings-heyframe-updates-plugins': {
                            template: '<div></div>',
                        },
                        'sw-loader': {
                            template: '<div></div>',
    ...
    it('should disable the button if one requirement is not met', async () => {
        const button = wrapper.findByText('button', 'sw-settings-heyframe-updates.infos.startUpdate');
    ...
    it('deactivate plugins success', async () => {
        ...
        expect(redirectSpy).toHaveBeenCalledWith(`${HeyFrame.Context.api.basePath}/heyframe-installer.phar.php`);
    });
    ...
    it('click on update button', async () => {
        ...
        await wrapper.get('.sw-settings-heyframe-updates-wizard__start-update').trigger('click');
        await flushPromises();

        expect(wrapper.vm.updateModalShown).toBe(true);

        expect(wrapper.find('.sw-settings-heyframe-updates-check__start-update').exists()).toBe(true);

        await wrapper.get('.sw-settings-heyframe-updates-check__start-update-backup-checkbox input').setChecked(true);

        const redirectSpy = jest.fn();
        wrapper.vm.redirectToPage = redirectSpy;

        await wrapper.get('.sw-settings-heyframe-updates-check__start-update-button').trigger('click');
