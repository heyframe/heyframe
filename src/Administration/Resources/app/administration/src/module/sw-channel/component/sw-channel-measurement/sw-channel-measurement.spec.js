...
async function createWrapper(repositoryMock, measurementUnits = {}, privileges = []) {
    ...
    return mount(await wrapTestComponent('sw-channel-measurement', { sync: true }), {
        props: {
            channel: {
                measurementUnits: {
                    system: 'metric',
...
describe('src/module/sw-channel/component/sw-channel-measurement', () => {
    beforeEach(() => {
    ...
    it('should use default labels when custom labels are not provided', async () => {
        ...
        expect(wrapper.vm.unitSystemLabel).toBe('sw-channel.detail.measurementSystem.labelUnitSystem');
        expect(wrapper.vm.dimensionUnitLabel).toBe('sw-channel.detail.measurementSystem.labelLengthUnit');
        expect(wrapper.vm.weightUnitLabel).toBe('sw-channel.detail.measurementSystem.labelWeightUnit');
    });
    ...
    it('should correctly update units on onMeasurementSystemChange', async () => {
        const wrapper = await createWrapper(null, {}, ['channel.editor']);
    ...
    it('should update lengthUnit prop on channel when sw-single-select for length changes', async () => {
        ...
        const wrapper = await createWrapper(measurementSystemRepository, {}, ['channel.editor']);
        await flushPromises();
        ...
        expect(wrapper.vm.channel.measurementUnits.units).toEqual({
            length: 'cm',
            weight: 'kg',
        });
    });

    it('should update weightUnit prop on channel when sw-single-select for weight changes', async () => {
        const measurementSystemRepository = {
            search: jest.fn(() => {
                return Promise.resolve(
                    new EntityCollection('/measurement-system', 'measurement_system', null, {}, [
        ...
        expect(wrapper.vm.channel.measurementUnits.units.weight).toBe('g');
    });
