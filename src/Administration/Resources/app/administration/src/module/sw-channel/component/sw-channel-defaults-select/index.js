...
import './sw-channel-defaults-select.scss';
import template from './sw-channel-defaults-select.html.twig';
...
export default {
    ...
    computed: {
        propertyCollection: {
            get() {
                ...
                return this.channel[this.propertyName];
            },
            set(newCollection) {
                if (!this.channel) {
                    return;
                }
                this.channel[this.propertyName] = newCollection;
            },
        },
        ...
        defaultId: {
            get() {
                ...
                return this.channel[this.defaultPropertyName];
            },
            set(newDefaultId) {
                if (this.channel) {
                    this.channel[this.defaultPropertyName] = newDefaultId;
                }
            },
        },
        ...
        propertyNameKebabCase() {
            return HeyFrame.Utils.string.kebabCase(this.propertyName);
        },

        multiSelectClass() {
            return `sw-channel-detail__select-${this.propertyNameKebabCase}`;
        },

        singleSelectClass() {
            return `sw-channel-detail__assign-${this.propertyNameKebabCase}`;
        },

        defaultsValueError() {
            return HeyFrame.Store.get('error').getApiError(this.channel, this.defaultPropertyName);
        },
    ...
    methods: {
            ...
            if (this.propertyNameInDomain) {
                ...
                if (domain !== null) {
                    this.createNotificationError({
                        message: this.$tc(
                            'sw-channel.sw-channel-defaults-select.messageError',
                            {
                                url: domain.url,
                            },
                            0,
        ...
        getDomainUsingValue(item) {
            return (
                this.channel.domains.find((domain) => {
