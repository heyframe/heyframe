...
import ChannelFavoritesService from 'src/module/sw-channel/service/channel-favorites.service';
        ...
        data: [
            {
                ...
                key: ChannelFavoritesService.USER_CONFIG_KEY,
                value: [],
            },
        ],
    },
...
describe('module/sw-channel/service/channel-favorites.service.spec.js', () => {
    let service;
    ...
    it('createUserConfigEntity > entity has specific values', async () => {
        const expectedValues = {
            userId: HeyFrame.Store.get('session').currentUser.id,
            key: ChannelFavoritesService.USER_CONFIG_KEY,
            value: [],
        };

        const entity = service.createUserConfigEntity(ChannelFavoritesService.USER_CONFIG_KEY);
    ...
    it('getCriteria > returns a criteria including specific filters', () => {
        const criteria = service.getCriteria(ChannelFavoritesService.USER_CONFIG_KEY);

        expect(criteria.filters).toContainEqual({
            type: 'equals',
            field: 'key',
            value: ChannelFavoritesService.USER_CONFIG_KEY,
        });
        expect(criteria.filters).toContainEqual({
            type: 'equals',
            field: 'userId',
