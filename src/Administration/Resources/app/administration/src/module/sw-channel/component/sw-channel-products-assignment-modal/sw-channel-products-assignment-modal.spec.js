...
async function createWrapper(activeTab = 'singleProducts') {
    return mount(
        await wrapTestComponent('sw-channel-products-assignment-modal', {
            sync: true,
        }),
        {
            global: {
                ...
                stubs: {
                    'sw-channel-products-assignment-single-products': true,
                    'sw-channel-product-assignment-categories': true,
                    'sw-channel-products-assignment-dynamic-product-groups': true,
                    'sw-container': {
...
describe('src/module/sw-channel/component/sw-channel-products-assignment-modal', () => {
    it('should emit modal close event', async () => {
        const wrapper = await createWrapper();

        await wrapper.get('.sw-channel-products-assignment-modal__close-button').trigger('click');
    ...
    it('should emit products data when clicking Add Products button to assign product individually', async () => {
        ...
        await wrapper.findByText('button', 'sw-channel.detail.products.buttonAddProducts').trigger('click');
    ...
    it('should emit products data when clicking Add Products button to assign product by categories', async () => {
        ...
        const assignButton = wrapper.findByText('button', 'sw-channel.detail.products.buttonAddProducts');
