/**
 * @sw-package discovery
 */

import { mount } from '@vue/test-utils';

async function createWrapper() {
    const systemConfigApiService = {
        getValues: jest.fn().mockResolvedValue({
            'core.measurementUnits.system': 'metric',
            'core.measurementUnits.length': 'mm',
            'core.measurementUnits.weight': 'kg',
        }),
    };

    return mount(await wrapTestComponent('sw-channel-create', { sync: true }), {
        global: {
            stubs: {
                'sw-page': {
                    template: `
                        <div class="sw-page">
                            <slot name="smart-bar-actions"></slot>
                        </div>
                    `,
                },
                'sw-button-process': {
                    template: '<button class="sw-button-process"></button>',
                    props: ['disabled'],
                },
                'sw-language-switch': true,
                'sw-card-view': true,
                'sw-language-info': true,
                'sw-tabs': true,
                'sw-tabs-item': true,
                'router-view': true,
                'sw-skeleton': true,
            },
            provide: {
                repositoryFactory: {
                    create: () => ({
                        create: () => ({}),
                        get: () =>
                            Promise.resolve({
                                productExports: {
                                    first: () => ({}),
                                },
                            }),
                        search: () => Promise.resolve([]),
                    }),
                },
                exportTemplateService: {
                    getProductExportTemplateRegistry: () => ({}),
                },
                systemConfigApiService,
            },
            mocks: {
                $route: {
                    params: {
                        id: '1a2b3c4d',
                        typeId: 'channel-type-id',
                    },
                    name: '',
                },
            },
        },
    });
}

describe('src/module/sw-channel/page/sw-channel-create', () => {
    beforeEach(() => {
        global.activeAclRoles = [];
    });

    it('should disable the save button when privilege does not exist', async () => {
        const wrapper = await createWrapper();
        const saveButton = wrapper.getComponent('.sw-channel-detail__save-action');

        await wrapper.setData({
            isLoading: false,
        });

        expect(saveButton.props('disabled')).toBe(true);
    });

    it('should enable the save button when privilege does exists', async () => {
        global.activeAclRoles = ['channel.creator'];
        const wrapper = await createWrapper();

        await wrapper.setData({
            isLoading: false,
        });

        const saveButton = wrapper.getComponent('.sw-channel-detail__save-action');

        expect(saveButton.props('disabled')).toBe(false);
    });

    it('should prepare measurementUnits for channel with values from system config', async () => {
        const wrapper = await createWrapper();

        expect(wrapper.vm.channel.measurementUnits).toEqual({
            system: 'metric',
            units: {
                length: 'mm',
                weight: 'kg',
            },
        });
    });
});
