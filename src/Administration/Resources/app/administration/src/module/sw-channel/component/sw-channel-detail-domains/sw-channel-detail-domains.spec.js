...
async function createWrapper(customProps = {}, domains = []) {
    return mount(
        await wrapTestComponent('sw-channel-detail-domains', {
            sync: true,
        }),
        {
            global: {
...
describe('src/module/sw-channel/component/sw-channel-detail-domains', () => {
    it('should have button and context menu item enabled', async () => {
        const wrapper = await createWrapper();

        const button = wrapper.find('.sw-channel-detail__button-domain-add');
    ...
    it('should only display available languages', async () => {
        ...
        expect(wrapper.getComponent('.sw-channel-detail-domains__domain-language-select').vm.value).toBe(
            languages.first().id,
        );
        expect([
            ...wrapper.getComponent('.sw-channel-detail-domains__domain-language-select').vm.$data.results,
    ...
    it('should only display available currencies', async () => {
        ...
        expect(wrapper.getComponent('.sw-channel-detail-domains__domain-currency-select').vm.value).toBe(
            currencies.first().id,
        );
        expect([
            ...wrapper.getComponent('.sw-channel-detail-domains__domain-currency-select').vm.$data.results,
