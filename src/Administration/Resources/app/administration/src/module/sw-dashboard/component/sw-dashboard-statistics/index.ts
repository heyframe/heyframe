 ...
 *
 * @private might get removed with any update (even minor!) as it likely gets replaced by heyframe analytics
 */
export default HeyFrame.Component.wrapComponentConfig({
    ...
    computed: {
            ...
            return {
                ...
                yaxis: {
                    ...
                    labels: {
                        ...
                        formatter: (value: string) =>
                            HeyFrame.Utils.format.currency(
                                Number.parseFloat(value),
                                HeyFrame.Context.app.systemCurrencyISOCode as string,
                                2,
                            ),
                    },
                },
        ...
        today() {
            const today = HeyFrame.Utils.format.dateWithUserTimezone();
        ...
        systemCurrencyISOCode() {
            return HeyFrame.Context.app.systemCurrencyISOCode;
        },

        isSessionLoaded() {
            return !HeyFrame.Store.get('session')?.userPending;
        },
    ...
    methods: {
        ...
        fetchHistory(paid: boolean, since: string) {
            ...
            const timezone = HeyFrame.Store.get('session').currentUser?.timeZone ?? 'UTC';
        ...
        formatDateToISO(date: Date) {
            return HeyFrame.Utils.format.toISODate(date, false);
        },

        formatChartHeadlineDate(date: Date) {
            const lastKnownLang = HeyFrame.Application.getContainer('factory').locale.getLastKnownLocale();
        ...
        getDateAgo(range: HistoryDateRange): Date {
            const date = HeyFrame.Utils.format.dateWithUserTimezone();
