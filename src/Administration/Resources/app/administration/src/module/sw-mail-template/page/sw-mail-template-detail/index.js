...
const { warn } = HeyFrame.Utils.debug;
...
export default {
    ...
    data() {
        return {
            ...
            testMailChannelId: null,
            availableVariables: {},
            entitySchema: Object.fromEntries(HeyFrame.EntityDefinition.getDefinitionRegistry()),
            showLanguageNotAssignedToChannelWarning: false,
        };
    },
    ...
    methods: {
        createdComponent() {
            ...
            HeyFrame.ExtensionAPI.publishData({
                id: 'sw-mail-template-detail__testMailChannelId',
                path: 'testMailChannelId',
                scope: this,
            });
            HeyFrame.ExtensionAPI.publishData({
        ...
        loadEntityData() {
            ...
            this.isLoading = true;
            this.mailTemplateRepository.get(this.mailTemplateId, HeyFrame.Context.api, criteria).then((item) => {
        ...
        createMediaCollection() {
            return new EntityCollection('/media', 'media', HeyFrame.Context.api);
        },
        ...
        getMailTemplateMedia() {
            ...
            this.mailTemplate.media.forEach((mediaAssoc) => {
                if (mediaAssoc.languageId === HeyFrame.Context.api.languageId) {
        ...
        onClickTestMailTemplate() {
            ...
            const notificationTestMailErrorChannel = {
                message: this.$tc('sw-mail-template.general.notificationTestMailChannelErrorMessage'),
            };

            if (!this.testMailChannelId) {
                this.createNotificationError(notificationTestMailErrorChannel);
                return;
            }
            ...
            this.channelRepository.get(this.testMailChannelId, Context.api, criteria).then((channel) => {
                if (!channel.languages.has(HeyFrame.Context.api.languageId)) {
                    this.showLanguageNotAssignedToChannelWarning = true;

                    return;
                }

                this.showLanguageNotAssignedToChannelWarning = false;
            });
        ...
        createMailTemplateMediaAssoc(mediaItem) {
            ...
            mailTemplateMedia.languageId = HeyFrame.Context.api.languageId;
        ...
        _checkIfMediaIsAlreadyUsed(mediaId) {
            return this.mailTemplate.media.some((mailTemplateMedia) => {
                return (
                    mailTemplateMedia.mediaId === mediaId && mailTemplateMedia.languageId === HeyFrame.Context.api.languageId
                );
            });
        },
        ...
        loadAvailableVariables(variable, variableEntitySchema) {
            ...
            const foundVariables = Object.keys(HeyFrame.Utils.get(this.mailTemplateType.templateData, variable));

            const keys = foundVariables.map((val) => {
                const availableVariable = HeyFrame.Utils.get(this.mailTemplateType.templateData, variablePath.concat(val));
        ...
        isToManyAssociationVariable(variable) {
            ...
            const field = HeyFrame.Utils.get(this.entitySchema, `${variables.join('.')}`);
        ...
        loadInitialAvailableVariables() {
            ...
            Object.keys(this.mailTemplateType.templateData).forEach((variable) => {
                const availableVariable = HeyFrame.Utils.get(this.mailTemplateType.templateData, variable);
                let length = 0;
