...
async function createWrapper(privileges = []) {
    return mount(await wrapTestComponent('sw-mail-template-detail', { sync: true }), {
        global: {
            ...
            mocks: {
                $route: { params: { id: HeyFrame.Utils.createId() } },
            },
            stubs: {
                'sw-page': {
                    template: `
...
describe('modules/sw-mail-template/page/sw-mail-template-detail', () => {
    ...
    it('should be able to send test mails when values are filled', async () => {
        ...
        await wrapper.setData({
            mailTemplate: {
                ...mailTemplateTypeMock,
                subject: 'Your order with {{ channel.name }} is partially paid',
                contentPlain: 'the status of your order at {{ channel.translated.name }}',
    ...
    it('should display an notification if content language is not assigned to selected sales channel', async () => {
        ...
        expect(wrapper.vm.showLanguageNotAssignedToChannelWarning).toBeFalsy();

        HeyFrame.Context.api.languageId = 'foo';
        await sendTestMail.trigger('click');

        await flushPromises();

        expect(wrapper.vm.showLanguageNotAssignedToChannelWarning).toBeTruthy();
    });
