        ...
        global: {
            ...
            stubs: {
                ...
                'sw-select-result': {
                    ...
                    methods: {
                        onClickResult() {
                            HeyFrame.Utils.EventBus.emit('item-select', this.item);
                        },
                    },
                },
...
describe('src/module/sw-order/view/sw-order-create-options', () => {
    beforeAll(() => {
        ...
        HeyFrame.Store.get('swOrder').setCart(cart);
        HeyFrame.Store.get('swOrder').setContext(context);
        HeyFrame.Store.get('swOrder').setCustomer(customerData);

        if (HeyFrame.Store.get('context')) {
            HeyFrame.Store.unregister('context');
        }

        HeyFrame.Store.register(contextStore);
    });
    ...
    it('should update context when selecting shipping method', async () => {
        ...
        HeyFrame.Service('cartStoreService').getCart = jest.fn(() =>
            Promise.resolve({
                data: {
                    lineItems: [],
                    deliveries: [
    ...
    it('should be set context language when language selected', async () => {
        ...
        expect(HeyFrame.Store.get('context').api.languageId).toBe(contextStore.state().api.languageId);
