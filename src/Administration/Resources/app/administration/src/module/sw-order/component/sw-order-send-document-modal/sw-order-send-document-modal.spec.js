...
const mockRepositoryFactory = (entity, mailTemplates) => {
    if (entity === 'mail_template') {
        return {
            search: jest.fn(() =>
                Promise.resolve(new EntityCollection('', '', HeyFrame.Context.api, null, mailTemplates, 2)),
            ),
    ...
    if (entity === 'mail_header_footer') {
        return {
            search: (criteria) => {
                ...
                return Promise.resolve(new EntityCollection('', '', HeyFrame.Context.api, null, [mockMailHeaderFooter], 1));
            },
        };
    }
    return {};
