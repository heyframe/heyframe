...
describe('src/module/sw-order/component/sw-order-create-address-modal', () => {
    ...
    it('should dispatch error with invalid company field', async () => {
        ...
        expect(HeyFrame.Store.get('error').api.customer_address).toBeUndefined();

        // submit form
        await findByText(swModalEditAddress, 'button', 'sw-customer.detailAddresses.buttonSaveAndSelect').trigger('click');

        expect(HeyFrame.Store.get('error').api).toHaveProperty('customer_address.12345.company');
        expect(HeyFrame.Store.get('error').api.customer_address['12345'].company).toBeInstanceOf(HeyFrameError);
    });
});
