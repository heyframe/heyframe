    ...
    return mount(await wrapTestComponent('sw-system-config'), {
        ...
        global: {
            ...
            stubs: {
                ...
                'sw-channel-switch': await wrapTestComponent('sw-channel-switch'),
            ...
            provide: {
                systemConfigApiService: {
                    ...
                    getValues: (domain, channelId) => {
                        if (defaultValues[domain] && defaultValues[domain][channelId]) {
                            return Promise.resolve(defaultValues[domain][channelId]);
                        }
...
function createConfig() {
    const firstCardElements = [
        ...
        },
        {
            ...
            type: 'url',
            config: {
                defaultValue: 'https://www.heyframe.com',
                label: {
                    'en-GB': 'url field',
                },
            },
            _test: {
                defaultValueDom: 'www.heyframe.com',
                domValueCheck: (field, domValue) => {
                    expect(field.find('input').element.value).toBe(domValue);
                },
                afterValue: 'https://www.heyframe.de',
                afterValueDom: 'www.heyframe.de',
                childValue: 'https://www.child.heyframe.com',
                childValueDom: 'www.child.heyframe.com',
                changeValueFunction: async (field, afterValue) => {
                    // change input value
...
function createEntityCollection(entities = []) {
    return new HeyFrame.Data.EntityCollection('collection', 'collection', {}, null, entities);
}
...
describe('src/module/sw-settings/component/sw-system-config/sw-system-config', () => {
    it('should show a select field for the sales channels', async () => {
        ...
        const selectionText = wrapper.find('#channelSelect .sw-entity-single-select__selection-text');
    ...
    it('should change the sales channel', async () => {
        ...
        let channelSwitch = wrapper.find('.sw-field[label="sw-settings.system-config.labelChannelSelect"]');
        let selectionText = channelSwitch.find('.sw-entity-single-select__selection-text');

        expect(selectionText.text()).toBe('sw-channel-switch.labelDefaultOption');

        // open channel switch field
        await channelSwitch.find('.sw-select__selection').trigger('click');
        await flushPromises();

        channelSwitch = wrapper.find('.sw-field[label="sw-settings.system-config.labelChannelSelect"]');

        // select headless sales channel
        const selectOptionTwo = channelSwitch.find('.sw-select-option--2');
    ...
    it('should return HeyFrameError when has error', async () => {
        HeyFrame.Store.get('error').addApiError({
            expression: 'SYSTEM_CONFIG.null.dummyKey',
            error: new HeyFrameError({ code: 'dummyCode' }),
        });
        ...
        expect(error).toBeInstanceOf(HeyFrameError);
    });
    ...
    createConfig()[0].elements.forEach(({ name, type, config, _test }) => {
        ...
        it(`should render field with type "${type || name}" with the inherit value and should be able to remove the inheritance`, async () => {
            ...
            const channelSwitch = wrapper.find('.sw-field[label="sw-settings.system-config.labelChannelSelect"]');
            let selectionText = channelSwitch.find('.sw-entity-single-select__selection-text');
