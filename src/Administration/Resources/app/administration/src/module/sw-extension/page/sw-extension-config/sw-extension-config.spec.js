...
describe('src/module/sw-extension/page/sw-extension-config.spec', () => {
                    ...
                    systemConfigApiService: {
                        getValues: () => {
                            return Promise.resolve({
                                'core.store.apiUri': 'https://api.heyframe.com',
                                'core.store.licenseHost': 'sw6.test.heyframe.in',
                                'core.store.shopSecret': 'very.s3cret',
                                'core.store.heyframeId': 'max@muster.com',
                            });
                        },
                    },
                },
    ...
    it('should reload extensions on createdComponent', async () => {
        ...
        expect(wrapper.vm.heyframeExtensionService.updateExtensionData).toHaveBeenCalledTimes(1);
    });

    it('should not reload extensions on createdComponent if extensions are loaded', async () => {
        HeyFrame.Store.get('heyframeExtensions').setMyExtensions([{ name: 'test-extension' }]);
        const wrapper = await createWrapper();

        expect(wrapper.vm.heyframeExtensionService.updateExtensionData).toHaveBeenCalledTimes(0);
    });
    ...
    it('shows header for extension details', async () => {
        ...
        const meta = wrapper.get('.sw-meteor-page__smart-bar-meta');
        expect(meta.text()).toBe('sw-extension-store.component.sw-extension-config.labelBy heyframe AG');
    });
    ...
    it('shows header for extension details with producer website', async () => {
        ...
        wrapper.vm.extension = {
            producerName: 'heyframe AG',
            producerWebsite: 'https://www.heyframe.com/',
        };
        ...
        expect(metaLink.attributes().href).toBe('https://www.heyframe.com/');
        expect(metaLink.text()).toBe('heyframe AG');
    });
