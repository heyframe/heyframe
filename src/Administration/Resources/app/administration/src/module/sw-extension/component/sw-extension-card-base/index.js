...
export default {
    ...
    computed: {
        ...
        extensionMainModule() {
            return HeyFrame.Store.get('extensionMainModules').mainModules.find(
        ...
        extensionManagementDisabled() {
            return HeyFrame.Store.get('context').app.config.settings?.disableExtensionManagement;
        },
    ...
    methods: {
        async createdComponent() {
            this.openLink = await this.heyframeExtensionService.getOpenLink(this.extension);
        },
        ...
        async closeModalAndUninstallExtension(removeData) {
            ...
            try {
                await this.heyframeExtensionService.uninstallExtension(this.extension.name, this.extension.type, removeData);
        ...
        async updateExtension(allowNewPermissions = false) {
            ...
            try {
                ...
                if (this.extension.installedAt) {
                    await this.heyframeExtensionService.updateExtension(
                        this.extension.name,
                        this.extension.type,
                        allowNewPermissions,
                    );
        ...
        async removeExtension(removeData) {
            try {
                ...
                await this.heyframeExtensionService.removeExtension(this.extension.name, this.extension.type, removeData);
