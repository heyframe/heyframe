...
import type { HeyFrameDiscountCampaignService } from 'src/app/service/discount-campaign.service';
import type {
 ...
 * @private
 */
export default class HeyFrameExtensionService {
    ...
    constructor(
        ...
        private readonly discountCampaignService: HeyFrameDiscountCampaignService,
    ...
    public async updateExtensionData(refreshExtensions: boolean = true): Promise<void> {
        HeyFrame.Store.get('heyframeExtensions').loadMyExtensions();

        try {
            if (!HeyFrame.Store.get('context').app.config?.settings?.disableExtensionManagement && refreshExtensions) {
    ...
    public async checkLogin(): Promise<void> {
        try {
            ...
            HeyFrame.Store.get('heyframeExtensions').userInfo = userInfo;
        } catch {
            HeyFrame.Store.get('heyframeExtensions').userInfo = null;
        }
    }
    ...
    public async getOpenLink(extension: Extension): Promise<null | LabeledLocation | RouteLocationNamedRaw> {
        ...
        const entryRoutes = HeyFrame.Store.get('extensionEntryRoutes').routes;
    ...
    private async getLinkToTheme(extension: Extension) {
        ...
        const themeRepository = HeyFrame.Service('repositoryFactory').create('theme');
    ...
    private getAppFromStore(extensionName: string) {
        return HeyFrame.Store.get('heyframeApps').apps.find((innerApp) => {
