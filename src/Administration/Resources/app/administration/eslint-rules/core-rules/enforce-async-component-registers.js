 ...
 *
 * Overview
 * This custom ESLint rule enforces that all HeyFrame.Component.register and Component.register calls
 ...
 *
 * // Invalid
 * HeyFrame.Component.register('sw-example', { template: '<div>Example</div>' });
 * Component.register('sw-example', import('./sw-example'));
 *
 * // Valid
 * HeyFrame.Component.register('sw-example', () => import('./sw-example'));
 * Component.register('sw-example', () => import('./sw-example'));
 *
 */
module.exports = {
    meta: {
        type: 'problem',
        docs: {
            description: 'enforce that HeyFrame.Component.register calls use an anonymous arrow function as the second argument',
    ...
    create(context) {
        return {
            CallExpression(node) {
                // Check if the callee is HeyFrame.Component.register or Component.register
                const isHeyFrameRegister =
                    node.callee.type === 'MemberExpression' &&
                    node.callee.object.type === 'MemberExpression' &&
                    node.callee.object.object.name === 'HeyFrame' &&
                ...
                if (!isHeyFrameRegister && !isComponentRegister) {
                    return;
                }
                ...
                if (node.arguments.length < 2) {
                    context.report({
                        ...
                        data: {
                            registerCall: isHeyFrameRegister ? 'HeyFrame.Component.register' : 'Component.register',
                        },
                    });
                    return;
                }
