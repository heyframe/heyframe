...
namespace HeyFrame\Core\Framework\Update\Api;
...
use HeyFrame\Core\Framework\Update\Checkers\LicenseCheck;
use HeyFrame\Core\Framework\Update\Checkers\WriteableCheck;
...
class UpdateController extends AbstractController
{
    ...
    public function updateApiCheck(): JsonResponse
    {
        ...
        if (version_compare($this->heyframeVersion, $updates->version, '>=')) {
            return new JsonResponse();
        }
    ...
    public function deactivatePlugins(Request $request, Context $context): JsonResponse
    {
        ...
        if ($offset === 0) {
            ...
            $this->eventDispatcher->dispatch(
                new UpdatePrePrepareEvent($context, $this->heyframeVersion, $update->version)
            );
        }
        ...
        if ($result->getOffset() === $result->getTotal()) {
            ...
            $containerWithoutPlugins->get('event_dispatcher')->dispatch(
                new UpdatePostPrepareEvent($context, $this->heyframeVersion, $update->version)
            );
        }
