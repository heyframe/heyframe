...
namespace HeyFrame\Core\Framework\Adapter\Cache\ReverseProxy;

use HeyFrame\Core\Framework\Adapter\Cache\Http\CacheStore;
...
class ReverseProxyCompilerPass implements CompilerPassInterface
{
    ...
    {
        if (!$container->getParameter('heyframe.http_cache.reverse_proxy.enabled')) {
        ...
        if ($container->getParameter('heyframe.http_cache.reverse_proxy.fastly.enabled')) {
