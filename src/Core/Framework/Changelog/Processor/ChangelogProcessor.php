...
namespace HeyFrame\Core\Framework\Changelog\Processor;

use HeyFrame\Core\Framework\Changelog\ChangelogFile;
use HeyFrame\Core\Framework\Changelog\ChangelogFileCollection;
use HeyFrame\Core\Framework\Changelog\ChangelogParser;
...
class ChangelogProcessor
{
    ...
    public function findLatestTag(): ?string
    {
        ...
        $result = shell_exec('gh release list -R heyframe/heyframe --exclude-drafts --exclude-pre-releases --json tagName,isLatest');
        if (!$result) {
            return null;
        }
    ...
    public function getFixCommits(string $fromRef): array
    {
        ...
        foreach ($refs as $ref) {
            ...
            if ($body && preg_match_all('/' . ChangelogParser::FIXES_REGEX . '/i', $body, $matches)) {
                ...
                if ($author && !$this->isHeyFrameOrgMember($author['login'])) {
                    $fix['author'] = $author;
                }
    ...
    {
        $result = shell_exec(\sprintf('gh pr view https://github.com/heyframe/heyframe/pull/%s --json author', escapeshellarg(ltrim($issueId, '#'))));
    ...
    private function isHeyFrameOrgMember(string $login): bool
    {
        if ($this->users === null) {
            $this->users = [];
            $result = shell_exec('gh api --paginate -H "Accept: application/vnd.github+json" -H "X-GitHub-Api-Version: 2022-11-28" /orgs/heyframe/members');
            if ($result) {
    ...
    private function getPlatformRoot(): string
    {
        if (!isset($this->platformRoot)) {
            ...
            if ($composerJson === null || $composerJson['name'] !== 'heyframe/platform') {
