...
The OpenAPI specification for the StoreAPI is generated by the `HeyFrame\Core\Framework\Api\ApiDefinition\Generator\StoreApiGenerator`. This generator builds the final schema by merging information from the next sources:

1.  **Entity Definitions:** it inspects all `EntityDefinition`s that are available for the StoreAPI (`ChannelDefinitionInterface`) to automatically generate base schemas for entities.
