 ...
 *
 * @phpstan-import-type ErrorData from HeyFrameHttpException as HeyFrameExceptionData
 */
#[Package('framework')]
class ErrorResponseFactory
{
    ...
    /**
     * @return array<DefaultExceptionData|HeyFrameExceptionData>
     */
    public function getErrorsFromException(\Throwable $exception, bool $debug = false): array
    {
        if ($exception instanceof HeyFrameHttpException) {
            $errors = [];
    ...
    private function getStatusCodeFromException(\Throwable $exception): int
    {
        ...
        if ($exception instanceof HeyFrameHttpException || $exception instanceof HttpException) {
