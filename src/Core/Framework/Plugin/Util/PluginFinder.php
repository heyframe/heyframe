...
use HeyFrame\Core\Framework\Plugin\Struct\PluginFromFileSystemStruct;
...
class PluginFinder
{
    ...
    private function loadLocalPlugins(string $pluginDir, IOInterface $composerIO, ExceptionCollection $errors): array
    {
        ...
        try {
            ...
            foreach ($filesystemPlugins as $filesystemPlugin) {
                ...
                if (!$this->isHeyFramePluginType($package) || !$this->isPluginComposerValid($package)) {
    ...
    private function isHeyFramePluginType(CompletePackageInterface $package): bool
    {
    ...
    private function enrichWithVendorPlugins(
        ...
        foreach ($composerPackages as $composerPackage) {
            if (!$this->isHeyFramePluginType($composerPackage)) {
                continue;
            }
        ...
        if ($this->isHeyFramePluginType($root) && $this->isPluginComposerValid($root)) {
