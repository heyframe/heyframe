...
class ImitateCustomerTokenGenerator
{
    ...
    public function generate(string $channelId, string $customerId, string $userId): string
    {
        $tokenData = [
            'channelId' => $channelId,
        ...
        if ($data === false) {
            throw CustomerException::invalidImitationToken($channelId . ':' . $customerId . ':' . $userId);
        }

        return $this->encrypt(hash_hmac(self::HMAC_HASH_ALGORITHM, $data, $this->appSecret) . '.' . time());
    }

    public function validate(string $givenToken, string $channelId, string $customerId, string $userId): void
    {
