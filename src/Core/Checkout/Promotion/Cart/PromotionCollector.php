...
use HeyFrame\Core\Checkout\Promotion\Gateway\PromotionGatewayInterface;
use HeyFrame\Core\Checkout\Promotion\Gateway\Template\PermittedAutomaticPromotions;
use HeyFrame\Core\Checkout\Promotion\Gateway\Template\PermittedGlobalCodePromotions;
use HeyFrame\Core\Checkout\Promotion\Gateway\Template\PermittedIndividualCodePromotions;
...
class PromotionCollector implements CartDataCollectorInterface
{
     ...
     */
    private function searchPromotionsAuto(CartDataCollection $data, ChannelContext $context): array
    {
        ...
        $criteria
            ->addFilter(new PermittedAutomaticPromotions($context->getChannelId()))
     ...
     * @throws InconsistentCriteriaIdsException
     */
    private function searchPromotionsByCodes(CartDataCollection $data, array $allCodes, ChannelContext $context): CartPromotionsDataDefinition
    {
        ...
        foreach ($codesToFetch as $currentCode) {
            ...
            $globalCriteria
                ->addFilter(new PermittedGlobalCodePromotions([$currentCode], $channelId))
                ->addAssociations(self::REQUIRED_DAL_ASSOCIATIONS);
            ...
            if ($foundPromotions->count() === 0) {
                ...
                $individualCriteria
                    ->addFilter(new PermittedIndividualCodePromotions([$currentCode], $channelId))
                    ->addAssociations(self::REQUIRED_DAL_ASSOCIATIONS);
     ...
     */
    private function buildDiscountLineItems(string $code, PromotionEntity $promotion, Cart $original, ChannelContext $context): array
    {
