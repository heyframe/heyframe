...
class ContainerCartProcessor implements CartProcessorInterface
{
    ...
    private function calculateCollection(LineItemCollection $items, ChannelContext $context, \Closure $condition): void
    {
        foreach ($items as $item) {
    ...
    private function calculate(LineItem $item, ChannelContext $context, LineItemCollection $scope): void
    {
        if ($item->getChildren()->count() > 0) {
