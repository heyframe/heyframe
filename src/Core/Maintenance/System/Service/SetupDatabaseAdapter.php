...
class SetupDatabaseAdapter
{
    ...
    public function initializeHeyFrameDb(Connection $connection, ?string $database = null): bool
    {
        if (!$hasHeyFrameTables = $this->hasHeyFrameTables($connection, $database)) {
            $connection->executeStatement($this->getBaseSchema());
        }

        return !$hasHeyFrameTables;
    }

    public function hasHeyFrameTables(Connection $connection, ?string $database = null): bool
    {
        if ($database) {
