...
class ShopConfigurator
{
    ...
    private function setSystemConfig(string $key, string $value): void
    {
        ...
        // Fetch id for config key, as the unique key on config_key and channelId will not work when channelId is null
        $id = $this->connection->fetchOne('
            SELECT `id`
            FROM `system_config`
            WHERE `configuration_key` = :key AND `channel_id` IS NULL
        ', ['key' => $key]);
        ...
        $this->connection->executeStatement('
            INSERT INTO `system_config` (`id`, `configuration_key`, `configuration_value`, `channel_id`, `created_at`)
