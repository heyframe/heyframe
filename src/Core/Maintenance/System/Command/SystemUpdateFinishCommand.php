...
use HeyFrame\Core\Framework\Update\Api\UpdateController;
use HeyFrame\Core\Framework\Update\Event\UpdatePostFinishEvent;
use HeyFrame\Core\Framework\Update\Event\UpdatePreFinishEvent;
...
class SystemUpdateFinishCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $this->eventDispatcher->dispatch(new UpdatePreFinishEvent($context, $oldVersion, $this->heyframeVersion));

        if (!$input->getOption('skip-migrations')) {
            $this->runMigrations($io, $input);
        }

        $updateEvent = new UpdatePostFinishEvent($context, $oldVersion, $this->heyframeVersion);
    ...
    private function runMigrations(HeyFrameStyle $io, InputInterface $input): void
    {
    ...
    private function installAssets(HeyFrameStyle $io): int
    {
        $application = $this->getConsoleApplication();
     ...
     */
    private function runCommand(Application $application, Command $command, array $arguments, HeyFrameStyle $io): int
    {
