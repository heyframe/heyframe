...
use HeyFrame\Core\Installer\Configuration\AdminConfigurationService;
use HeyFrame\Core\Installer\Configuration\EnvConfigWriter;
use HeyFrame\Core\Installer\Configuration\ShopConfigurationService;
 ...
 *
 * @phpstan-import-type SupportedLanguages from \HeyFrame\Core\Installer\Controller\InstallerController
 */
#[Package('framework')]
class ShopConfigurationController extends InstallerController
{
    ...
    public function shopConfiguration(Request $request): Response
    {
        ...
        $preselection = $this->container->getParameter('heyframe.installer.configurationPreselection');
    ...
    private function getCountryIsos(Connection $connection, string $currentLocale): array
    {
        ...
        $countryIsos = array_map(fn ($country) => [
            ...
            'translated' => $this->translator->trans('heyframe.installer.select_country_' . mb_strtolower((string) $country['iso3'])),
        ], $countries);
