...
use HeyFrame\Core\Installer\Requirements\Struct\RequirementCheck;
use HeyFrame\Core\Installer\Requirements\Struct\RequirementsCheckCollection;
use HeyFrame\Core\Installer\Requirements\Struct\SystemCheck;
...
class EnvironmentRequirementsValidator implements RequirementsValidatorInterface
{
    ...
    {
        $platform = $this->composer->getRepositoryManager()->getLocalRepository()->findPackage('heyframe/platform', '*');
        if (!$platform) {
            $platform = $this->composer->getRepositoryManager()->getLocalRepository()->findPackage('heyframe/core', '*');
        }
        if (!$platform) {
