...
use HeyFrame\Core\Content\Sitemap\Event\SitemapFilterOpenTagEvent;
...
class SitemapHandle implements SitemapHandleInterface
{
    ...
    {
        return 'sitemap/channel-' . $channelContext->getChannelId() . '-' . $channelContext->getLanguageId() . '/';
    }
    ...
    private function getFileName(ChannelContext $channelContext, ?int $index = null): string
    {
        if ($this->domainName === null) {
            return \sprintf($channelContext->getChannelId() . '-' . self::SITEMAP_NAME_PATTERN, null, $index ?? $this->index);
        }

        if ($this->domainId === null) {
            return \sprintf($channelContext->getChannelId() . '-' . self::SITEMAP_NAME_PATTERN, '-' . $this->domainName, $index ?? $this->index);
        }

        return \sprintf($channelContext->getChannelId() . '-' . $this->domainId . '-' . self::SITEMAP_NAME_PATTERN, '-' . $this->domainName, $index ?? $this->index);
    }
