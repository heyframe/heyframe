...
trait MediaFixtures
{
    ...
    public function initializeMediaFixtures(): void
    {
        ...
        $channelRepository = static::getFixtureRepository('channel');

        $stateId = $stateRepository->search(new Criteria(), $this->entityFixtureContext)->getEntities()->first()?->getId() ?? Uuid::randomHex();
        $channelId = $channelRepository->search(new Criteria(), $this->entityFixtureContext)->getEntities()->first()?->getId() ?? Uuid::randomHex();
