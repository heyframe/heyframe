...
Current HeyFrame CMS stores `cms_page_id` directly on entities but lacks sales channel dimensionality. Same product must use different layouts in different sales channels without duplicating entities. Dynamic resolution queries assignment table with entity ID + sales channel context.
...
If route.layout_cascade exists:
  query content_layout_assignment WHERE:
    ...
    - channel_id matches context (with fallback to null)
