...
- `channel_id` (nullable) - Sales channel scope or NULL for global
...
LayoutResolver loads assignments for route WHERE:
  ...
  - channel_id = {context} OR channel_id IS NULL

Sort by:
  - priority DESC
  - channel_id DESC (channel-specific over global)
...
WHERE (
    ...
    OR channel_id IS NULL  -- Global assignments
)
ORDER BY channel_id DESC  -- Channel-specific before global
```

Global assignments (NULL channel_id) work across all channels but rank lower than channel-specific.

## Query Details

Single query per resolution:
1. Filter by route_id and sales channel
2. Sort by priority DESC, channel_id DESC
