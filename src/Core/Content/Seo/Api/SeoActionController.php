...
namespace HeyFrame\Core\Content\Seo\Api;

use HeyFrame\Core\Content\Seo\ConfiguredSeoUrlRoute;
use HeyFrame\Core\Content\Seo\Exception\NoEntitiesForPreviewException;
use HeyFrame\Core\Content\Seo\SeoException;
use HeyFrame\Core\Content\Seo\SeoUrl\SeoUrlEntity;
use HeyFrame\Core\Content\Seo\SeoUrlGenerator;
...
use HeyFrame\Core\Content\Seo\Validation\SeoUrlDataValidationFactoryInterface;
use HeyFrame\Core\Content\Seo\Validation\SeoUrlValidationFactory;
...
class SeoActionController extends AbstractController
{
    ...
    private function getPreview(array $seoUrlTemplate, Context $context, ?Criteria $previewCriteria = null): array
    {
        ...
        if (\is_string($channelId)) {
            $channel = $this->channelRepository->search((new Criteria([$channelId]))->setLimit(1), $context)->getEntities()->get($channelId);
