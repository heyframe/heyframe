...
class ProductFeatureBuilder
{
     ...
     */
    public function prepare(iterable $lineItems, CartDataCollection $data, ChannelContext $context): void
    {
     ...
     */
    public function add(iterable $lineItems, CartDataCollection $data, ChannelContext $context): void
    {
     ...
     */
    private function buildFeatures(CartDataCollection $data, LineItem $lineItem, ChannelProductEntity $product): array
    {
     ...
     * @param iterable<LineItem> $lineItems
     */
    private function loadCustomFields(iterable $lineItems, CartDataCollection $data, ChannelContext $context): void
    {
        $required = [];
        ...
        foreach ($lineItems as $lineItem) {
            ...
            $product = $data->get($this->getDataKey($productId));
            if (!$product instanceof ChannelProductEntity || $product->getCustomFields() === null) {
                continue;
            }
     ...
     */
    private function isRequiredCustomField(string $name, ChannelProductEntity $product): bool
    {
     ...
     */
    private function getAttribute(string $name, ChannelProductEntity $product): array
    {
     ...
     * @return ?array{label: string, value: mixed, type: string}
     */
    private function getProperty(string $id, ChannelProductEntity $product): ?array
    {
     ...
     */
    private function getCustomField(string $name, CartDataCollection $data, ChannelProductEntity $product): ?array
    {
     ...
     */
    private function getReferencePrice(LineItem $lineItem, ChannelProductEntity $product): ?array
    {
     ...
     *
     * @see https://github.com/heyframe/heyframe/issues/4458
     */
