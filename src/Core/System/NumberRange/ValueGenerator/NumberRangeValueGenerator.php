...
use HeyFrame\Core\System\NumberRange\Exception\NoConfigurationException;
use HeyFrame\Core\System\NumberRange\NumberRangeEvents;
use HeyFrame\Core\System\NumberRange\ValueGenerator\Pattern\ValueGeneratorPatternRegistry;
...
class NumberRangeValueGenerator implements NumberRangeValueGeneratorInterface
{
    ...
    {
        $config = $this->getConfiguration($type, $channelId);
        ...
        return $this->endEvent($generatedValue, $type, $context, $channelId, $preview);
    }
    ...
    private function endEvent(string $generatedValue, string $type, Context $context, ?string $channelId, bool $preview): string
    {
        $generatedEvent = $this->eventDispatcher->dispatch(
            new NumberRangeGeneratedEvent($generatedValue, $type, $context, $channelId, $preview),
     ...
     */
    private function getConfiguration(string $definition, ?string $channelId): array
    {
        if ($channelId) {
            ...
            $config = $this->connection->fetchAssociative('
                ...
                LEFT JOIN number_range_channel ON number_range.id = number_range_channel.number_range_id
                WHERE `number_range_type`.`technical_name` = :typeName AND (
                    number_range_channel.channel_id = :channelId OR number_range_type.global = 1 OR number_range.global = 1
                )
                ORDER BY number_range.global ASC, number_range_type.global ASC
            ', ['typeName' => $definition, 'channelId' => Uuid::fromHexToBytes($channelId)]);
        } else {
            /** @var array{id: string, pattern: string, start: ?int}|false $config */
            $config = $this->connection->fetchAssociative('
                SELECT LOWER(HEX(`number_range`.`id`)) AS `id`, `number_range`.`pattern`, `number_range`.`start`
        ...
        if (!$config) {
            throw new NoConfigurationException($definition, $channelId);
        }
