...
use HeyFrame\Core\Framework\DataAbstractionLayer\FilteredBulkEntityExtension;
...
class ChannelEntityCompilerPass implements CompilerPassInterface
{
    ...
    private function collectDefinitions(ContainerBuilder $container): void
    {
        ...
        $channelDefinitions = $this->formatData(
            $container->findTaggedServiceIds('heyframe.channel.entity.definition'),
            $container
        );

        $baseDefinitions = $this->formatData(
            $container->findTaggedServiceIds('heyframe.entity.definition'),
            $container
        );
    ...
    private function addExtensions(ContainerBuilder $container, array $baseEntityDefinitions, array $channelDefinitions): void
    {
        ...
        foreach ($container->findTaggedServiceIds('heyframe.entity.extension') as $id => $tags) {
        ...
        foreach ($container->findTaggedServiceIds('heyframe.bulk.entity.extension') as $id => $tags) {
            $definition = $container->getDefinition($id);
