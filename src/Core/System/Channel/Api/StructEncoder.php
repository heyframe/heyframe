...
use HeyFrame\Core\System\Channel\Entity\DefinitionRegistryChain;
...
class StructEncoder implements ResetInterface
{
    ...
    public function encode(Struct $struct, ResponseFields $fields): array
    {
        ...
        if (!\is_array($array)) {
            throw ChannelException::encodingInvalidStructException('Normalized struct must be an array');
        }
        ...
        if ($struct instanceof AggregationResultCollection) {
            ...
            foreach (\array_keys($struct->getElements()) as $index => $key) {
                if (!isset($data[$index]) || !\is_array($data[$index])) {
                    throw ChannelException::encodingMissingAggregationException($key, $index);
                }

                $entity = $struct->get($key);
                if (!$entity instanceof Struct) {
                    throw ChannelException::encodingInvalidStructException(\sprintf('Aggregation "%s" is not a valid struct', $key));
                }
        ...
        if ($struct instanceof EntitySearchResult) {
            ...
            if (isset($data['elements'])) {
                ...
                foreach (\array_values($data['elements']) as $index => $value) {
                    ...
                    if (!$entity instanceof Struct) {
                        throw ChannelException::encodingInvalidStructException(\sprintf('Entity at index "%d" is not a valid struct', $index));
                    }
