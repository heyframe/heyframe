...
class SnippetService
{
    ...
    public function findSnippetSetId(string $channelId, string $languageId, string $locale): string
    {
        $snippetSetId = $this->connection->fetchOne(
            'SELECT LOWER(HEX(`snippet_set`.`id`))
            FROM `channel_domain`
            INNER JOIN `snippet_set` ON `channel_domain`.`snippet_set_id` = `snippet_set`.`id`
            WHERE `channel_domain`.`channel_id` = :channelId AND `channel_domain`.`language_id` = :languageId
            LIMIT 1',
            [
