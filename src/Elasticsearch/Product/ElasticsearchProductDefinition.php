...
use HeyFrame\Core\Framework\DataAbstractionLayer\Dbal\SqlHelper;
...
use HeyFrame\Elasticsearch\Framework\ElasticsearchFieldBuilder;
use HeyFrame\Elasticsearch\Framework\ElasticsearchFieldMapper;
use HeyFrame\Elasticsearch\Framework\ElasticsearchIndexingUtils;
...
class ElasticsearchProductDefinition extends AbstractElasticsearchDefinition
{
    ...
    public function getMapping(Context $context): array
    {
        ...
        $channelByLanguage = $this->languageLoader->loadLanguages();
        $allChannels = array_values(array_unique(array_merge(...array_values($channelByLanguage))));

        $visibilities = [];

        foreach ($allChannels as $channelId) {
            $visibilities['visibility_' . $channelId] = [
                'type' => 'integer',
            ];
        }
    ...
    public function fetch(array $ids, Context $context): array
    {
        ...
        foreach ($data as $id => $item) {
            ...
            foreach ($visibilities as $key => $visibility) {
                ...
                $visibilitiesFlatten['visibility_' . $visibility['channelId']] = $visibility['visibility'] ?? 0;
            }
...
SQL;
        ...
        $baseMapping = [
            ...
            '#visibilities#' => SqlHelper::objectArray([
                ...
                'channelId' => 'LOWER(HEX(product_visibility.channel_id))',
            ], 'visibilities'),
        ];
