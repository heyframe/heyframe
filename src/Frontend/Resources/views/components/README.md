...
namespace HeyFrame\Frontend\Resources\views\components\Button;
...
<service id="HeyFrame\Frontend\Resources\views\components\Button">
...
**Note:** Be carefull with adding too much business logic inside component classes. At HeyFrame we mostly implement anonymous components that are pure frontend representaions which get passed all necessary data via properties. 


## Adding components via HeyFrame extension

New components can easiley be added by HeyFrame apps and plugins in the same way as described above.

### Anonymous Components
The anonymous components work right of the box and you dont't have to do anything else than putting your component files under `<your-plugin>/src/Resources/views/components`. Both, apps and plugins can provide anonymous components in any distribution form of HeyFrame. Your components will automatically be prefixed with the bundle name of your app or plugin. 
...
The APIs of a component are "Props", "Blocks", "Attributes" and "Slots". Slots are an additional feature in heyframe to allow component extension using the CMS.
