...
1. **HeyFrame** - A new global singleton which acts as a central entry point.
2. **HeyFrameComponent** - The abstract base class for all components.

## Component Architecture

### Base Component Class

All components extend from the `HeyFrameComponent` base class, which provides:
...
### Global HeyFrame Instance

The `HeyFrame` class acts as a singleton that:
...
The component has to extend the `HeyFrameComponent` class, which is globally available.

```javascript
class MyComponent extends HeyFrameComponent {
...
window.HeyFrame.registerComponent('my-component', MyComponent);
```

### Component Registration

Components are registered with the global HeyFrame instance. You can do this simply in the same file as your component.
...
Component scripts are automatically loaded when the corresponding Twig component is used within the page. Your component script should have the same name as the Twig file of your component and should be placed in the same directory. HeyFrame will automatically collect all component script files and include them into the page if the component is used on a specific page.
...
To react to actions from other components, there is a new central event system available which can be accessed via the global `window.HeyFrame` singleton.
...
class MyComponent extends HeyFrameComponent {
    ...
    doSomething() {
        ...
        window.HeyFrame.emit('MyComponent:DoSomething', message);
    }
}
```
...
class SomeOtherComponent extends HeyFrameComponent {

    init() {
        window.HeyFrame.on('MyComponent:DoSomething', (message) => {
...
class BuyButton extends HeyFrameComponent {
    ...
    onFormSubmit(event) {
        ...
        window.HeyFrame.emit('BuyButton:Submit', requestUrl, formData);

        window.HeyFrame.callPluginMethod('OffCanvasCart', 'openOffCanvas', requestUrl, formData);
    }
}
```
...
window.HeyFrame.intercept('BuyButton:PreSubmit', (data) => {
...
HeyFrame.callMethod('MyComponent', 'refresh');

// Get all instances of a component
const instances = HeyFrame.getComponentInstances('MyComponent');

// Get a specific instance by element
const instance = HeyFrame.getComponentInstanceByElement('MyComponent', element);
```
...
```javascript
class ReactiveComponent extends HeyFrameComponent {
    init() {
