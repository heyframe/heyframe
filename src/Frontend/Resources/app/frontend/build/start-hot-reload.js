    ...
    on: {
        ...
        error: (err, req, res) => {
            ...
            if (err.code === 'SSL_ERROR_NO_CYPHER_OVERLAP') {
                ...
                console.error('STOREFRONT_HTTPS_KEY_FILE=/var/www/html/.../certs/heyframe.key STOREFRONT_HTTPS_CERTIFICATE_FILE=/var/www/html/../certs/heyframe.crt composer run watch:storefront');
                process.exit(1);
            }
...
if (appUrlEnv.protocol === 'https:' && !sslFilesFound) {
    ...
    console.error('If you use a TLS proxy (like in DDEV HeyFrame 6 setup), you can ignore this message.');
}
...
server.then(() => {
    console.log('############');
    console.log(`Default TWIG Frontend: ${appUrlEnv.origin}`);
