...
const { getObjectDiff, cloneDeep, deepMergeObject } = HeyFrame.Utils.object;
const { isArray } = HeyFrame.Utils.types;
...
Component.register('sw-theme-manager-detail', {
    ...
    methods: {
        ...
        getTheme() {
            ...
            this.themeRepository.get(this.themeId, HeyFrame.Context.api, criteria).then((response) => {
                this.theme = response;
                this.getThemeConfig();
        ...
        onSaveTheme(clean = false) {
            ...
            return Promise.all([this.saveChannels(), this.saveThemeConfig(clean)]).then(() => {
                this.getTheme();
                ...
                if (errorObject.code === 'THEME__INVALID_SCSS_VAR') {
                    ...
                    error.response.data.errors.forEach((error) => {
                        ...
                        // See GitHub issue: https://github.com/heyframe/meteor/issues/906
        ...
        getHelpText(field) {
            ...
            const locale = HeyFrame.Store.get('session').currentLocale;
