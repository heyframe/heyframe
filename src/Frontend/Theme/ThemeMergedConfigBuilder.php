...
class ThemeMergedConfigBuilder
{
    ...
    public function getPlainThemeConfiguration(string $themeId, Context $context): array
    {
        ...
        $baseTheme = $this->themes->filter(fn (ThemeEntity $themeEntry) => $themeEntry->getTechnicalName() === FrontendPluginRegistry::BASE_THEME_NAME)->first();
        if ($baseTheme === null) {
    ...
    private function getTranslations(string $themeId, Context $context): array
    {
        ...
        if ($theme->getTechnicalName() !== FrontendPluginRegistry::BASE_THEME_NAME) {
            $criteria = (new Criteria())
