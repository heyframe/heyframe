...
class ThemeLifecycleService
{
     ...
     */
    private function createMediaStruct(FrontendPluginConfiguration $pluginConfig, string $path, string $mediaId, ?string $themeFolderId): ?array
    {
     ...
     */
    private function getTranslationsConfiguration(FrontendPluginConfiguration $configuration, Context $context): array
    {
    ...
    private function updateMediaInConfiguration(
        ?ThemeEntity $theme,
        FrontendPluginConfiguration $pluginConfiguration,
        Context $context
    ): array {
        $media = [];
        $themeData = [];
        ...
        if ($theme && \is_array($theme->getThemeJson())) {
            $fs = $this->themeFilesystemResolver->getFilesystemForFrontendConfig($pluginConfiguration);
     ...
     */
    private function getParentThemes(FrontendPluginConfiguration $config, string $id): array
    {
        ...
        $parentThemeConfigs = $allThemeConfigs->filter(
            fn (FrontendPluginConfiguration $parentConfig) => $this->isDependentTheme($parentConfig, $config)
        );

        $technicalNames = $parentThemeConfigs->map(
            fn (FrontendPluginConfiguration $theme) => $theme->getTechnicalName()
        );
    ...
    private function isDependentTheme(
        FrontendPluginConfiguration $parentConfig,
        FrontendPluginConfiguration $currentThemeConfig
    ): bool {
