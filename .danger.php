...
const COMPOSER_PACKAGE_EXCEPTIONS = [
    ...
    'strict' => [
        ...
        '^heyframe\/conflicts$' => 'The heyframe conflicts packages should be required in any version, so use `*` constraint',
        '^heyframe\/core$' => 'The heyframe core packages should be required in any version, so use `*` constraint, the version constraint will be automatically synced during the release process',
...
return (new Config())
    ...
    ->useRule(function (Context $context): void {
        ...
        if ($files->matches('*/heyframe.yaml')->count() > 0) {
            $context->warning('You updated the heyframe.yaml, please consider to update the config-schema.json');
        }
    })
    ->useRule(function (Context $context): void {
