...
# Returns a list of mandatory assets for the Frontend package.
frontend_assets_list() {
  cat <<EOF | tr -d '[:blank:]'
    ${PLATFORM_DIR}/repos/frontend/Resources/app/frontend/dist/frontend/frontend.js
    ${PLATFORM_DIR}/repos/frontend/Resources/app/frontend/vendor/bootstrap/package.json
EOF
}

check_frontend_assets() {
    stat -t $(frontend_assets_list) > /dev/null

    [[ "$(find ${PLATFORM_DIR}/repos/frontend/Resources/public/administration/assets/ -iname "*.js"  | wc -l)" -gt 0 ]]
    [[ "$(find ${PLATFORM_DIR}/repos/frontend/Resources/public/administration/assets/ -iname "*.css"  | wc -l)" -gt 0 ]]
}
...
check_assets() {
  ...
  if [[ ${package} == "" || ${package} == "frontend" ]]; then
    check_frontend_assets
  fi
...
require_core_version() {
  ...
  if [ "${package_lower}" != "core" ]; then
    ...
    composer -d "${PLATFORM_DIR}/repos/${package_lower}" require "heyframe/core:${package_version}" --no-update --no-install
  fi
}
...
# Removes certain asset-related entries from the frontend .gitignore.
include_frontend_assets() {
  sed -i -E '/[/]?Resources[/]app[/]frontend[/]vendor([/]?|.*)/d' "${PLATFORM_DIR}/repos/frontend/.gitignore"
  sed -i -E '/[/]?app[/]frontend[/]dist([/]?|.*)/d' "${PLATFORM_DIR}/repos/frontend/Resources/.gitignore"
  sed -i -E '/[/]?public([/]?|.*)/d' "${PLATFORM_DIR}/repos/frontend/Resources/.gitignore"
}

include_assets() {
  local package; package=$(lowercase "${1}")

  if [[ ${package} == "administration" || ${package} == "frontend" ]]; then
    copy_assets "${package}"
